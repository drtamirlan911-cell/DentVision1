# DentVision Platform Specification

**Статус:** Canonical Product Specification  
**Версия:** 1.0.1  
**Дата:** 2026-07-20  
**Уровень:** Product + Platform + Company Foundation

Это не README и не черновик идей.  
Это **операционная спецификация**, по которой можно проектировать продукт, команду, архитектуру и стратегию на годы вперёд.

---

## CORE — начать здесь

1. **[`MISSION.md`](./MISSION.md)** — Mission · CORE / CRITICAL  
2. **[`../00_CONSTITUTION/02_PRODUCT_DNA.md`](../00_CONSTITUTION/02_PRODUCT_DNA.md)** — Product DNA · CORE / CRITICAL  

> DentVision is an AI Operating System for Digital Dentistry.  
> DentVision is not software. DentVision is the digital operating system of dentistry.

Любая фича сначала проходит **Mission Decision Rule**, затем **Product DNA** (MUST/SHOULD gates, AI-primary interface, Apple/Linear quality bar).

Constitution index: [`../00_CONSTITUTION/README.md`](../00_CONSTITUTION/README.md)

---

## Как пользоваться

| Роль | С чего начать |
|------|----------------|
| Founder / CEO | [MISSION](./MISSION.md), [00 — Overview](./00-OVERVIEW.md), [13 — Roadmap](./13-ROADMAP-AND-HARDWARE.md) |
| Product | [MISSION](./MISSION.md), [02 — Personas](./02-PERSONAS-AND-ROLES.md), модули 05–09 |
| Design / UX | [03 — First Run](./03-FIRST-RUN-EXPERIENCE.md), [04 — AI](./04-AI-INTELLIGENCE.md) |
| Engineering | [04 — AI](./04-AI-INTELLIGENCE.md), [10 — Platform Core](./10-PLATFORM-CORE.md), [12 — NFR](./12-NON-FUNCTIONAL.md) |
| Security / Legal | [11 — Data & Privacy](./11-DATA-PRIVACY-SECURITY.md) |
| Growth / Ops | [14 — Success Metrics](./14-SUCCESS-METRICS.md) |

---

## Оглавление

- **[MISSION — Core Statement & Decision Rule](./MISSION.md)** ← CRITICAL
0. [Overview & Document Control](./00-OVERVIEW.md)
1. [Vision & Product Principles](./01-VISION-AND-PRINCIPLES.md)
2. [Personas, Roles & Jobs-to-be-Done](./02-PERSONAS-AND-ROLES.md)
3. [First-Run Experience](./03-FIRST-RUN-EXPERIENCE.md)
4. [AI Intelligence System](./04-AI-INTELLIGENCE.md)
5. [CRM (Workspace)](./05-CRM.md)
6. [Marketplace (Shop)](./06-MARKETPLACE.md)
7. [School (Academy)](./07-SCHOOL.md)
8. [Community](./08-COMMUNITY.md)
9. [Jobs](./09-JOBS.md)
10. [Platform Core](./10-PLATFORM-CORE.md)
11. [Data, Privacy & Security](./11-DATA-PRIVACY-SECURITY.md)
12. [Non-Functional Requirements](./12-NON-FUNCTIONAL.md)
13. [Roadmap & Hardware Future](./13-ROADMAP-AND-HARDWARE.md)
14. [Success Metrics & Company OS](./14-SUCCESS-METRICS.md)

---

## Связанные документы

| Документ | Роль |
|----------|------|
| [`/BLUEPRINT.md`](../../BLUEPRINT.md) | Архитектурный blueprint V2 (слои платформы) |
| [`/UX_BLUEPRINT.md`](../../UX_BLUEPRINT.md) | UX-карта экранов и motion-принципы |
| [`/README.md`](../../README.md) | Инженерный onboarding репозитория |
| [`/docs/MODULE_STATUS.md`](../MODULE_STATUS.md) | Текущий статус реализации модулей |

**Правило приоритета:**  
1. Constitution — [`MISSION`](./MISSION.md) + [`Product DNA`](../00_CONSTITUTION/02_PRODUCT_DNA.md)  
2. Остальная спецификация `docs/spec/`  
3. UX Blueprint → Blueprint → код  

---

## Product North Star (одно предложение)

> DentVision is an AI Operating System for Digital Dentistry — one intelligence unifying clinics, doctors, labs, suppliers, education, community, jobs, analytics, hardware and cloud.
