...
// ═══════════════════════════════════════════════════════════════
// BRIDGE ROUTES — map frontend REST paths to server routes
// ═══════════════════════════════════════════════════════════════

// AI query bridge (frontend sends /api/ai/query with { text, history }, server has /api/ai/chat with { message, history })
...
// Generic CRM bridge: /api/{resource} → /api/crm/:clinicId/{resource}
...
// GET list
app.get('/api/patients', authenticate, (req, res, next) => {
  req.params.clinicId = req.user?.clinicId;
  req.url = `/${req.user?.clinicId}/patients${req.search || ''}`;
  crmRouter(req, res, next);
});
app.get('/api/appointments', authenticate, (req, res, next) => {
  req.params.clinicId = req.user?.clinicId;
  req.url = `/${req.user?.clinicId}/appointments${req.search || ''}`;
  crmRouter(req, res, next);
});
app.get('/api/billing/invoices', authenticate, (req, res, next) => {
  req.params.clinicId = req.user?.clinicId;
  req.url = `/${req.user?.clinicId}/receipts${req.search || ''}`;
  crmRouter(req, res, next);
});
app.get('/api/inventory', authenticate, (req, res, next) => {
  req.params.clinicId = req.user?.clinicId;
  req.url = `/${req.user?.clinicId}/inventory${req.search || ''}`;
  crmRouter(req, res, next);
});

// POST create/upsert
app.post('/api/patients', authenticate, (req, res, next) => {
  req.params.clinicId = req.user?.clinicId;
  req.url = `/${req.user?.clinicId}/patients`;
  crmRouter(req, res, next);
});
app.post('/api/appointments', authenticate, (req, res, next) => {
  req.params.clinicId = req.user?.clinicId;
  req.url = `/${req.user?.clinicId}/appointments`;
  crmRouter(req, res, next);
});
app.post('/api/billing/invoices', authenticate, (req, res, next) => {
  req.params.clinicId = req.user?.clinicId;
  req.url = `/${req.user?.clinicId}/receipts`;
  crmRouter(req, res, next);
});
app.post('/api/inventory', authenticate, (req, res, next) => {
  req.params.clinicId = req.user?.clinicId;
  req.url = `/${req.user?.clinicId}/inventory`;
  crmRouter(req, res, next);
});

// DELETE by id
app.delete('/api/patients/:id', authenticate, (req, res, next) => {
  req.params.clinicId = req.user?.clinicId;
  req.url = `/${req.user?.clinicId}/patients/${req.params.id}`;
  crmRouter(req, res, next);
});
app.delete('/api/appointments/:id', authenticate, (req, res, next) => {
  req.params.clinicId = req.user?.clinicId;
  req.url = `/${req.user?.clinicId}/appointments/${req.params.id}`;
  crmRouter(req, res, next);
});
app.delete('/api/inventory/:id', authenticate, (req, res, next) => {
  req.params.clinicId = req.user?.clinicId;
  req.url = `/${req.user?.clinicId}/inventory/${req.params.id}`;
  crmRouter(req, res, next);
});

// Single clinic GET (frontend calls /api/clinics/:id)
...
// Patient medical card (by patient ID)
