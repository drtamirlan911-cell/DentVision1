...
vi.mock('../lib/prisma.js', () => ({
  default: {
    payment: { findMany: findMany, updateMany: paymentUpdateMany },
    order: { findUnique, updateMany: orderUpdateMany },
    $transaction: transaction,
  },
}));
vi.mock('../modules/payments/kaspi.provider.js', () => ({
  providers: { kaspi_qr: { getPaymentStatus: providerStatus } },
}));
vi.mock('../modules/payments/payments.routes.js', () => ({
  claimPaymentForSettlement: claim,
  settlePaidPayment: settle,
}));
vi.mock('../modules/shop/checkout.compensation.js', () => ({
  compensateDeterministicCheckoutFailure: compensate,
}));
...
beforeEach(() => {
  ...
  transaction.mockImplementation(async (fn: any) => fn({ payment: { updateMany: paymentUpdateMany }, order: { updateMany: orderUpdateMany, findUnique: vi.fn().mockResolvedValue({ meta: {} }) } }));
...
describe('payment reconciliation', () => {
  ...
  it('settles a provider-confirmed paid payment exactly through the existing settlement path', async () => {
  ...
  it('converts a confirmed provider failure to payment_failed and compensates the checkout', async () => {
  ...
  it('does not inspect a fresh unknown payment before the grace period', async () => {
    const freshNow = basePayment.updatedAt.getTime() + 10_000;
    const result = await reconcileUnknownCheckoutPayments(freshNow);
