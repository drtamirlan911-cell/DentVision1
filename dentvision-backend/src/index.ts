...
import { startPayoutReadinessCronInterval } from './jobs/payoutReadinessCron.js';
...
async function main() {
  ...
  await runOnceMigration('cross_clinic_access', 'Cross-clinic access tables', async (tx) => {
    await tx.$executeRawUnsafe(`
      DO $$
      BEGIN
    ...
    // encryptions of the same value never match — iinHash is a deterministic
    // HMAC blind index, the only way "does this person have records at
    // another clinic" can ever be a queryable equality lookup.
    await tx.$executeRawUnsafe(`ALTER TABLE "patients" ADD COLUMN IF NOT EXISTS "iinHash" VARCHAR(64)`);
    await tx.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "patients_iinHash_idx" ON "patients"("iinHash")`);
    await tx.$executeRawUnsafe(`
    ...
    `);
    await tx.$executeRawUnsafe(`CREATE UNIQUE INDEX IF NOT EXISTS "cross_clinic_access_grants_patientUserId_sourceClinicId_re_key" ON "cross_clinic_access_grants"("patientUserId", "sourceClinicId", "receivingClinicId")`);
    await tx.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "cross_clinic_access_grants_patientUserId_status_idx" ON "cross_clinic_access_grants"("patientUserId", "status")`);
    await tx.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "cross_clinic_access_grants_receivingClinicId_status_idx" ON "cross_clinic_access_grants"("receivingClinicId", "status")`);
    await tx.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "cross_clinic_access_grants_receivingPatientId_idx" ON "cross_clinic_access_grants"("receivingPatientId")`);
    await tx.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "cross_clinic_access_grants_sourceClinicId_idx" ON "cross_clinic_access_grants"("sourceClinicId")`);
    await tx.$executeRawUnsafe(`
      DO $$
      BEGIN
    ...
    `);
    await tx.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "cross_clinic_access_logs_grantId_createdAt_idx" ON "cross_clinic_access_logs"("grantId", "createdAt")`);
    await tx.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "cross_clinic_access_logs_accessedByUserId_idx" ON "cross_clinic_access_logs"("accessedByUserId")`);
    await tx.$executeRawUnsafe(`
      DO $$
      BEGIN
