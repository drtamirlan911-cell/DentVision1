...
import { assertPatientBranchBelongsToClinic, canAccessPatientBranch, getPatientBranchId, getPatientIdsForBranchScope, resolvePatientBranchContext, setPatientBranch } from '../../lib/patientBranchScope.js';
...
async function branchContext(req: AuthRequest, clinicId: string) {
  return resolvePatientBranchContext(req.user!.id, clinicId, String(req.user!.role));
}

async function scopedPatientWhere(req: AuthRequest, clinicId: string, extra: Prisma.PatientWhereInput = {}): Promise<Prisma.PatientWhereInput> {
  const context=await branchContext(req,clinicId);
  if(context.scope.kind==='organization') return {clinicId,...extra};
  return getScopedWhere(clinicId,context.scope.branchIds,extra);
}

async function getScopedWhere(clinicId: string, branchIds: readonly string[], extra: Prisma.PatientWhereInput): Promise<Prisma.PatientWhereInput> {
  if(!branchIds.length)return {clinicId,id:'__NO_BRANCH_ACCESS__',...extra};
  const ids=await getPatientIdsForBranchScope(clinicId,branchIds);
  return {clinicId,id:{in:ids},...extra};
}

async function loadClinicPatient(req:AuthRequest,res:any){const clinicId=req.user?.clinicId;if(!clinicId){res.status(400).json({ok:false,error:'Клиника не указана'});return null}const p=await prisma.patient.findFirst({where:{id:req.params.id,clinicId},select:{id:true}});if(!p){res.status(404).json({ok:false,error:'Пациент не найден'});return null}const context=await branchContext(req,clinicId);const patientBranchId=await getPatientBranchId(p.id,clinicId);if(context.scope.kind!=='organization'&&!canAccessPatientBranch(context,patientBranchId)){res.status(404).json({ok:false,error:'Пациент не найден'});return null}return{clinicId,patientId:p.id,context,branchId:patientBranchId};}
...
patientsRouter.get('/', async (req: AuthRequest, res) => {
  try {
    ...
    const whereBase:Prisma.PatientWhereInput=iinHash?{iinHash}:search?{OR:[{firstName:{contains:search,mode:'insensitive'}},{lastName:{contains:search,mode:'insensitive'}},{phone:{contains:search,mode:'insensitive'}},{email:{contains:search,mode:'insensitive'}}]}:{};
    const where=await scopedPatientWhere(req,clinicId,whereBase);
...
patientsRouter.post('/', requirePermission('patient.write'), guardPatientCreate, async (req: AuthRequest,res) => {
  ...
  try {
    const clinicId=req.user?.clinicId; if(!clinicId) return res.status(400).json({ok:false,error:'Клиника не указана'} satisfies ApiResponse);
    const context=await branchContext(req,clinicId); const body=req.body||{}, {firstName,lastName}=splitName(body.name,body.firstName,body.lastName), id=body.id||uid();
    let targetBranchId:string|null=context.branchId;
    if(context.scope.kind==='organization'){if(body.branchId!==undefined){if(!(await assertPatientBranchBelongsToClinic(String(body.branchId),clinicId)))return res.status(400).json({ok:false,error:'Недействующий филиал'} satisfies ApiResponse);targetBranchId=String(body.branchId);}else{const branches=await prisma.$queryRaw<Array<{id:string}>>`SELECT id FROM branches WHERE clinic_id=${clinicId} AND active=true ORDER BY "isDefault" DESC, "createdAt" ASC LIMIT 1`;targetBranchId=branches[0]?.id??null;}}
    else if(!targetBranchId)return res.status(403).json({ok:false,error:'Сотруднику не назначен филиал'} satisfies ApiResponse);
    const history:Record<string,unknown>={...(body.medicalHistory&&typeof body.medicalHistory==='object'?body.medicalHistory:{})}; for(const k of ['category','source','allergies','tags','teeth'])if(body[k])history[k]=body[k];
    if(!body.id){idempotencyKey=req.headers['idempotency-key'] as string|undefined;if(!idempotencyKey)idempotencyKey=`server-${createHash('sha256').update(`${req.user!.id}:${clinicId}:${JSON.stringify(body)}`).digest('hex').slice(0,32)}`;const r=await reserveIdempotencyKey(idempotencyKey);if(r.status==='in_flight')return res.status(409).json({ok:false,error:'Пациент уже создаётся, повторите позже'} satisfies ApiResponse);if(r.status==='exists'){const prior=await prisma.patient.findUnique({where:{id:r.resultId},include:{teeth:true}});if(prior){const priorBranch=await getPatientBranchId(prior.id,clinicId);if(context.scope.kind!=='organization'&&!canAccessPatientBranch(context,priorBranch))return res.status(404).json({ok:false,error:'Пациент не найден'});return res.json({ok:true,data:serializePatient(prior)} satisfies ApiResponse)}await deleteIdempotencyKey(idempotencyKey)}}
    const existing=body.id?await prisma.patient.findFirst({where:{id:body.id,clinicId}}):null;
    if(existing){const existingBranch=await getPatientBranchId(existing.id,clinicId);if(context.scope.kind!=='organization'&&!canAccessPatientBranch(context,existingBranch))return res.status(404).json({ok:false,error:'Пациент не найден'});if(context.scope.kind==='organization'&&body.branchId!==undefined){if(!(await assertPatientBranchBelongsToClinic(String(body.branchId),clinicId)))return res.status(400).json({ok:false,error:'Недействующий филиал'});targetBranchId=String(body.branchId)}else targetBranchId=existingBranch;}
    const iinTouched=body.iin!==undefined||body.noIinReason!==undefined; let iinFields:PatientIinFields|undefined;if(!existing||iinTouched)iinFields=await buildPatientIinFields({iin:body.iin,noIinReason:body.noIinReason,clinicId,excludePatientId:existing?.id,birthDate:(body.dob||body.birthDate)??existing?.birthDate??null,gender:body.gender??existing?.gender??null,required:!existing});
