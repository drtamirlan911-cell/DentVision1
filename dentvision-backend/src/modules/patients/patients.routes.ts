...
async function scopedPatientWhere(req: AuthRequest, clinicId: string, extra: Record<string, unknown> = {}) {
  const context = await branchContext(req, clinicId);
  if (context.scope.kind === 'organization') return { clinicId, ...extra };
  if (!context.scope.branchIds.length) return { clinicId, id: '__NO_BRANCH_ACCESS__', ...extra };
  const ids = await getPatientIdsForBranchScope(clinicId, context.scope.branchIds);
  return { clinicId, id: { in: ids }, ...extra };
}

async function loadClinicPatient(req: AuthRequest, res: any) {
  const clinicId=req.user?.clinicId;
  if(!clinicId){res.status(400).json({ok:false,error:'Клиника не указана'});return null;}
  const p=await prisma.patient.findFirst({where:{id:req.params.id,clinicId},select:{id:true}});
  if(!p){res.status(404).json({ok:false,error:'Пациент не найден'});return null;}
  const context=await branchContext(req,clinicId);
  const patientBranchId=await getPatientBranchId(p.id,clinicId);
  if(context.scope.kind!=='organization'&&!canAccessPatientBranch(context,patientBranchId)){
    res.status(404).json({ok:false,error:'Пациент не найден'});return null;
  }
  return {clinicId,patientId:p.id,context,branchId:patientBranchId};
}
...
patientsRouter.get('/', async (req: AuthRequest, res) => {
  try {
    ...
    const whereBase={...(iinHash?{iinHash}:search?{OR:[{firstName:{contains:search,mode:'insensitive' as const}},{lastName:{contains:search,mode:'insensitive' as const}},{phone:{contains:search,mode:'insensitive' as const}},{email:{contains:search,mode:'insensitive' as const}}]}:{})};
  ...
  try {
    ...
    if(context.scope.kind==='organization'){
      if(body.branchId!==undefined){
        if(!(await assertPatientBranchBelongsToClinic(String(body.branchId),clinicId))) return res.status(400).json({ok:false,error:'Недействующий филиал'} satisfies ApiResponse);
        targetBranchId=String(body.branchId);
      } else {
        const branches=await prisma.$queryRaw<Array<{id:string}>>`SELECT id FROM branches WHERE clinic_id=${clinicId} AND active=true ORDER BY is_default DESC, created_at ASC LIMIT 1`;
        targetBranchId=branches[0]?.id??null;
      }
    } else if(!targetBranchId) {
      return res.status(403).json({ok:false,error:'Сотруднику не назначен филиал'} satisfies ApiResponse);
    }
    ...
    if(targetBranchId) await setPatientBranch(patient.id,clinicId,targetBranchId);
    ...
    else await auditFromReq(req,{action:'patient.updated',entity:'patient',entityId:patient.id,details:{name:`${firstName} ${lastName}`.trim(),branchId:targetBranchId}});
