...
export async function createCenter(data: { name: string; city?: string; address?: string; phone?: string; email?: string; }) {
  const id = uid();
  const center = await prisma.diagnosticCenter.create({ data: { id, ...data } });
...
export async function createLaboratory(data: { name: string; city?: string; address?: string; phone?: string; email?: string; }) {
  const id = uid();
  const lab = await prisma.laboratory.create({ data: { id, ...data } });
...
export async function createRegistrationRequest(data: {
  type: 'center' | 'laboratory'; name: string; city?: string; address?: string; phone?: string; email?: string; comment?: string; userId?: string;
}) {
  return prisma.registrationRequest.create({
    data: { id: uid(), type: data.type, name: data.name, city: data.city, address: data.address, phone: data.phone, email: data.email, comment: data.comment, userId: data.userId, status: 'PENDING' },
  });
}

export const DIAGNOSTICS_ROLE_TO_ROLE_KEY: Record<string, string> = { owner: 'owner', admin: 'admin', manager: 'manager' };

export async function grantDiagnosticsAccess(type: 'DiagnosticCenter' | 'Laboratory', entityId: string, userId: string, role: string) {
  try {
    ...
    await syncOrgFromEntity(type, entityId, { name: entity.name, city: entity.city, address: entity.address, phone: entity.phone, email: entity.email });
    const org = await prisma.organization.findFirst({ where: { originalType: type, originalId: entityId } });
    if (!org) return false;
    if (type === 'DiagnosticCenter') {
      await prisma.diagnosticCenterMember.upsert({ where: { centerId_userId: { centerId: entityId, userId } }, update: { role }, create: { id: uid(), centerId: entityId, userId, role } });
    } else {
      await prisma.laboratoryMember.upsert({ where: { labId_userId: { labId: entityId, userId } }, update: { role }, create: { id: uid(), labId: entityId, userId, role } });
    }
    ...
    let person = await prisma.person.findFirst({ where: { originalType, originalId: `${entityId}:${userId}` } });
    if (person) {
      person = await prisma.person.update({ where: { id: person.id }, data: { fullName, personType: 'STAFF', organizationId: org.id, originalType, originalId: `${entityId}:${userId}` } });
    } else {
      person = await prisma.person.create({ data: { id: uid(), fullName, personType: 'STAFF', organizationId: org.id, userId: user.id, originalType, originalId: `${entityId}:${userId}` } });
    }
    const roleKey = DIAGNOSTICS_ROLE_TO_ROLE_KEY[String(role).toLowerCase()];
    if (!roleKey) {
      console.warn(`[Diagnostics] No unified role for ${type} member role '${role}' (user ${userId}) — PersonRole not assigned`);
      return true;
    }
...
export async function listRegistrationRequests(status?: string) {
  ...
  return prisma.registrationRequest.findMany({ where, orderBy: { createdAt: 'desc' } });
}
...
export async function approveRegistrationRequest(id: string, reviewerId: string) {
  ...
  if (req.type === 'center') {
    const centerId = uid();
    await prisma.diagnosticCenter.create({ data: { id: centerId, name: req.name, city: req.city, address: req.address, phone: req.phone, email: req.email, active: true } });
    ...
    const labId = uid();
    await prisma.laboratory.create({ data: { id: labId, name: req.name, city: req.city, address: req.address, phone: req.phone, email: req.email, active: true } });
  ...
  return prisma.registrationRequest.update({ where: { id }, data: { status: 'APPROVED', reviewerId, updatedAt: new Date() } });
}
...
export async function rejectRegistrationRequest(id: string, reviewerId: string, reason?: string) {
  ...
  if (!req) throw new Error('Registration request not found');
  if (req.status !== 'PENDING') throw new Error('Request already processed');
  return prisma.registrationRequest.update({ where: { id }, data: { status: 'REJECTED', reviewerId, reviewNote: reason, updatedAt: new Date() } });
}
...
const referralInclude = {
  ...
  result: { select: { id: true, aiGenerated: true, createdAt: true, reportText: true, conclusion: true, pdfUrl: true, signedBy: true, signedAt: true } },
...
export async function getReferral(id: string) {
  return prisma.referral.findUnique({ where: { id }, include: { ...referralInclude, comments: { include: { author: { select: { id: true, firstName: true, lastName: true } } }, orderBy: { createdAt: 'asc' } } } });
}

export async function createReferral(data: {
  patientName: string; patientIin?: string; patientBirth?: string; patientGender?: string; patientPhone?: string; patientEmail?: string; pregnancy?: boolean; allergies?: string; specialNotes?: string;
  clinicId: string; doctorId: string; doctorName?: string; doctorPhone?: string; doctorEmail?: string; category: DiagnosticCategory; studyType: string; anatomicalSites?: any;
  complaints?: string; preliminaryDx?: string; studyGoal?: string; commentForDoctor?: string; commentForLab?: string; priority?: ReferralPriority; centerId?: string; labId?: string; patientId?: string;
}, userId: string) {
  if (!data.clinicId) {
    const today = new Date(); today.setHours(0, 0, 0, 0);
    const count = await prisma.referral.count({ where: { doctorId: userId, createdAt: { gte: today } } });
  ...
  const referral = await prisma.referral.create({
    data: {
      id: uid(), patientName: data.patientName, patientIin, patientBirth: data.patientBirth ? new Date(data.patientBirth) : undefined, patientGender: data.patientGender,
      patientPhone: data.patientPhone, patientEmail: data.patientEmail, pregnancy: data.pregnancy, allergies: data.allergies, specialNotes: data.specialNotes,
      clinicId: data.clinicId, doctorId: data.doctorId, doctorName: data.doctorName, doctorPhone: data.doctorPhone, doctorEmail: data.doctorEmail,
      category: data.category, studyType: data.studyType, anatomicalSites: data.anatomicalSites || undefined, complaints: data.complaints, preliminaryDx: data.preliminaryDx,
      studyGoal: data.studyGoal, commentForDoctor: data.commentForDoctor, commentForLab: data.commentForLab, priority: data.priority, centerId: data.centerId, labId: data.labId, patientId: data.patientId,
      status: (data.centerId || data.labId) ? 'SENT' : 'DRAFT',
    },
  });
  await writeAuditLog({ action: 'REFERRAL_CREATED', entity: 'referral', entityId: referral.id, details: { category: data.category, studyType: data.studyType }, userId, clinicId: data.clinicId });
  publish('referral.created', { referralId: referral.id, clinicId: data.clinicId || '', centerId: data.centerId || '', doctorId: userId, patientName: data.patientName || '', studyType: data.studyType || '', status: (data.centerId || data.labId) ? 'SENT' : 'DRAFT', userId });
  if (data.centerId) await createNotificationForCenter(data.centerId, { type: NOTIFICATION_TYPES.REFERRAL_SENT, title: 'Новое направление', message: `${data.patientName || 'Пациент'} — ${data.studyType || 'исследование'}`, link: `/center-workspace?tab=referrals` });
  return referral;
}
...
export async function updateReferral(id: string, data: any, userId: string) {
  ...
  await writeAuditLog({ action: 'REFERRAL_UPDATED', entity: 'referral', entityId: id, details: { updated: Object.keys(data) }, userId, clinicId: referral.clinicId });
  return referral;
}
...
export async function changeReferralStatus(id: string, status: ReferralStatus, userId: string, reason?: string, cost?: number, platformFee?: number) {
  ...
  if (status === 'ACCEPTED' && referral.centerId) {
    ...
    if (!sub.active) throw new Error('Подписка диагностического центра истекла. Для приёма направлений требуется активная подписка (20 000 ₸/мес).');
  }
  ...
  if (status === 'ACCEPTED' || status === 'IN_PROGRESS') {
    ...
    if (resolvedCost === undefined && referral.centerId && referral.studyType) {
      try {
        const study = await prisma.diagnosticStudy.findFirst({ where: { centerId: referral.centerId, active: true, name: { contains: referral.studyType, mode: 'insensitive' } }, select: { price: true } });
  ...
  if (status === 'COMPLETED' && updatedReferral.centerId && updatedReferral.cost != null) {
    await recordPartnerEconomics({
      vertical: 'DIAGNOSTIC_3D',
      partnerId: updatedReferral.centerId,
      grossMinor: BigInt(Math.round(Number(updatedReferral.cost) * 100)),
      operationId: updatedReferral.id,
      branchId: updatedReferral.branchId ?? null,
    });
  }
  ...
  await writeAuditLog({ action: 'REFERRAL_STATUS_CHANGED', entity: 'referral', entityId: id, details: { status, reason }, userId, clinicId: updatedReferral.clinicId });
  publish(`referral.${status.toLowerCase()}` as any, { referralId: id, clinicId: updatedReferral.clinicId || '', centerId: updatedReferral.centerId || '', doctorId: updatedReferral.doctorId || '', patientName: updatedReferral.patientName || '', studyType: updatedReferral.studyType || '', status, userId, cost: updatedReferral.cost, platformFee: updatedReferral.platformFee });
...
export async function deleteReferral(id: string, userId: string) {
  ...
  await writeAuditLog({ action: 'REFERRAL_DELETED', entity: 'referral', entityId: id, userId, clinicId: referral.clinicId });
}

// ─── Files ───

export async function uploadReferralFile(data: { referralId: string; fileName: string; fileData: string; fileType: string; fileSize?: number; uploadedBy: string; }) {
  const file = await prisma.referralFile.create({ data: { id: uid(), referralId: data.referralId, fileName: data.fileName, fileUrl: data.fileData, fileType: data.fileType, fileSize: data.fileSize, uploadedBy: data.uploadedBy } });
  await writeAuditLog({ action: 'FILE_UPLOADED', entity: 'referral', entityId: data.referralId, details: { fileName: data.fileName, fileType: data.fileType }, userId: data.uploadedBy });
  return file;
}
...
export async function deleteReferralFile(id: string, userId: string) {
  ...
  await writeAuditLog({ action: 'FILE_DELETED', entity: 'referral', entityId: file.referralId, details: { fileName: file.fileName }, userId });
}

// ─── Comments ───

export async function addComment(referralId: string, authorId: string, text: string) {
  return prisma.referralComment.create({ data: { id: uid(), referralId, authorId, text }, include: { author: { select: { id: true, firstName: true, lastName: true } } } });
}
...
export async function getDashboardStats(scope: { clinicId?: string; centerId?: string; labId?: string } = {}) {
  ...
  const [total, todayCount, pending, completed, overdue, recent] = await Promise.all([
    ...
    prisma.referral.findMany({ where, include: referralInclude, orderBy: { createdAt: 'desc' }, take: 10 }),
  ]);
...
export class DiagnosticAiUnavailable extends Error {
  ...
  constructor(reason: string, message: string) { super(message); this.name = 'DiagnosticAiUnavailable'; this.reason = reason; }
}
...
const REPORT_SCHEMA = {
  ...
  schema: { type: 'object', additionalProperties: false, required: ['description', 'findings', 'conclusion', 'recommendations'], properties: {
    description: { type: 'string' }, findings: { type: 'array', items: { type: 'string' } }, conclusion: { type: 'string' }, recommendations: { type: 'array', items: { type: 'string' } },
  } },
} as const;
interface StructuredReport { description: string; findings: string[]; conclusion: string; recommendations: string[]; }
function renderReport(report: StructuredReport): string {
  const list = (items: string[]) => items.filter(Boolean).map((i) => `- ${i}`).join('\n') || '- не выявлено';
  return [`**Описание исследования**\n${report.description}`, `**Находки**\n${list(report.findings)}`, `**Заключение**\n${report.conclusion}`, `**Рекомендации**\n${list(report.recommendations)}`].join('\n\n');
}
function parseReport(raw: string): StructuredReport | null {
  try {
    ...
    return { description: parsed.description, findings: Array.isArray(parsed.findings) ? parsed.findings.map(String) : [], conclusion: parsed.conclusion, recommendations: Array.isArray(parsed.recommendations) ? parsed.recommendations.map(String) : [] };
  } catch { return null; }
}

async function requireViewableSource(referral: { clinicId: string; patientId: string | null; files?: Array<{ fileUrl: string | null; fileName: string | null }> }): Promise<string> {
  if (!referral.patientId) throw new DiagnosticAiUnavailable('NO_PATIENT', 'Направление не связано с пациентом — согласие на анализ снимка проверить невозможно');
  const consent = await checkImageAnalysisConsent(referral.clinicId, referral.patientId);
  if (isImageConsentDenied(consent)) throw new DiagnosticAiUnavailable(consent.reason, IMAGE_CONSENT_MESSAGE[consent.reason]);
  const viewable = (referral.files || []).find(isViewable);
  if (!viewable?.fileUrl) throw new DiagnosticAiUnavailable('NO_VIEWABLE_FILE', 'К направлению не приложен снимок в читаемом формате (JPG, PNG, GIF, WEBP) — заключение по неувиденному исследованию не выдаётся');
  return resolveImageUrl(viewable.fileUrl);
}

export async function aiGenerateResult(referralId: string, userId: string) {
  const referral = await prisma.referral.findUnique({ where: { id: referralId }, include: { clinic: { select: { name: true } }, files: { select: { fileUrl: true, fileName: true } } } });
  if (!referral) throw new Error('Referral not found');
...
/**
 * Confirm an AI-generated diagnostic result without rewriting the report.
 * Confirmation is a clinical acknowledgement: the AI draft remains intact,
 * the confirming user is recorded as signer, the referral is completed, and
 * the action is auditable. Patient-card insertion remains limited to the
 * existing signed-result workflow and is never performed by the AI itself.
 */
export async function confirmAiResult(referralId: string, userId: string) {
  const referral = await prisma.referral.findUnique({
    where: { id: referralId },
    include: { result: true },
  });
  if (!referral) throw new Error('Referral not found');
  if (!referral.result) throw new Error('AI result not found');
  ...
  const now = new Date();
  const result = await prisma.diagnosticResult.update({
    where: { referralId },
    data: { signedBy: userId, signedAt: now },
  });
  ...
  await writeAuditLog({
    action: 'AI_RESULT_CONFIRMED',
    entity: 'referral',
    entityId: referralId,
    details: { resultId: result.id, confirmedBy: userId, aiGenerated: true },
    userId,
    clinicId: referral.clinicId,
  });

  publish('diagnostics.result_confirmed', {
    referralId,
    resultId: result.id,
    clinicId: referral.clinicId || '',
  ...
  return { result, referral: updatedReferral, confirmedBy: userId };
}
...
export async function seedTestData() {
  ...
  const center1 = await prisma.diagnosticCenter.create({ data: { id: uid(), name: 'DentaScan Алматы', city: 'Алматы', address: 'ул. Толе би, 185', phone: '+77011234567', email: 'info@dentascan.kz', active: true, rating: 4.8 } });
  const center2 = await prisma.diagnosticCenter.create({ data: { id: uid(), name: '3D Диагностика Астана', city: 'Астана', address: 'пр. Қабанбай батыра, 15', phone: '+77021234567', email: 'info@3dastana.kz', active: true, rating: 4.6 } });
  const center3 = await prisma.diagnosticCenter.create({ data: { id: uid(), name: 'МРТ-Эксперт Шымкент', city: 'Шымкент', address: 'ул. Байтурсынова, 10', phone: '+77031234567', email: 'info@mrt-shym.kz', active: true, rating: 4.5 } });
  const lab1 = await prisma.laboratory.create({ data: { id: uid(), name: 'LabaLab Алматы', city: 'Алматы', address: 'ул. Абая, 52', phone: '+77011235555', email: 'info@labalab.kz', active: true, rating: 4.7 } });
  const lab2 = await prisma.laboratory.create({ data: { id: uid(), name: 'БиоТест Астана', city: 'Астана', address: 'ул. Сарайшык, 5', phone: '+77021235555', email: 'info@biotest.kz', active: true, rating: 4.4 } });
  const studies = [
    { name: 'Конусно-лучевая КТ (КЛКТ)', category: 'CBCT' as const, centerId: center1.id }, { name: 'Панорамный снимок (ОПТГ)', category: 'OPG' as const, centerId: center1.id }, { name: 'Телерентгенограмма (ТРГ)', category: 'TRG' as const, centerId: center1.id }, { name: 'МРТ височно-нижнечелюстного сустава', category: 'TMJ' as const, centerId: center1.id },
    { name: 'Конусно-лучевая КТ (КЛКТ)', category: 'CBCT' as const, centerId: center2.id }, { name: 'Панорамный снимок (ОПТГ)', category: 'OPG' as const, centerId: center2.id }, { name: 'МРТ челюстно-лицевой области', category: 'TMJ' as const, centerId: center2.id },
    { name: 'Конусно-лучевая КТ (КЛКТ)', category: 'CBCT' as const, centerId: center3.id }, { name: 'Панорамный снимок (ОПТГ)', category: 'OPG' as const, centerId: center3.id },
  ];
  for (const s of studies) await prisma.diagnosticStudy.create({ data: { id: uid(), ...s } });
  const tests = [
    { name: 'Клинический анализ крови', category: 'BLOOD' as const, labId: lab1.id }, { name: 'Биохимический анализ крови', category: 'BLOOD' as const, labId: lab1.id }, { name: 'Гистологическое исследование', category: 'HISTOLOGY' as const, labId: lab1.id }, { name: 'Анализ на ВИЧ', category: 'BLOOD' as const, labId: lab1.id },
    { name: 'Микробиологический посев', category: 'MICROBIOLOGY' as const, labId: lab2.id }, { name: 'Цитологическое исследование', category: 'PATHOLOGY' as const, labId: lab2.id }, { name: 'Аллергопроба', category: 'ALLERGY' as const, labId: lab2.id },
  ];
  for (const t of tests) await prisma.laboratoryTest.create({ data: { id: uid(), ...t } });
  return { centers: [center1.name, center2.name, center3.name], labs: [lab1.name, lab2.name], studies: studies.length, tests: tests.length };
}
