...
async function buildSignInPayload(user: SignInUser, req: any, res: any) {
  ...
  const scopedRole = clinicId
    ...
    : authContext.organizationId
      ...
      : user.role;
  return { user: { ...userWithoutPassword, clinicId, organizationId: authContext.organizationId, organizationType: authContext.organizationType, organizationName: authContext.organizationName, personType: authContext.personType, effectiveRole: scopedRole, name: `${user.firstName} ${user.lastName}`.trim() }, memberships: memberships.map((m) => ({ id: m.id, role: m.role, clinicId: m.clinicId, joinedAt: m.joinedAt, clinic: m.clinic })), activeMembership, permissions: effectivePermissions, pages: pagesForCaller(effectivePermissions, scopedRole), capabilities: capabilitiesForPermissions(effectivePermissions, scopedRole), effectiveRole: scopedRole, ...tokens };
}
