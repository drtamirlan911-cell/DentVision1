...
async function membership(userId: string, clinicId: string) {
  const user = await prisma.user.findUnique({ where: { id: userId }, select: { role: true } });
  if (user?.role === 'SUPERADMIN') return { role: 'OWNER' as const, branchId: null as string | null };
...
// GET /api/branches?clinicId=...
// OWNER/ADMIN see every branch; branch-scoped staff see only their assigned branch.
branchesRouter.get('/', async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const rows = member.branchId && !canManage(member.role)
      ? await prisma.$queryRaw<BranchRow[]>`
          SELECT * FROM "branches"
          WHERE "clinic_id" = ${clinicId} AND "id" = ${member.branchId}
...
branchesRouter.post('/', async (req: AuthRequest, res) => {
  const { clinicId, code, name, city, address, phone, settings } = req.body as {
    clinicId?: string; code?: string; name?: string; city?: string; address?: string; phone?: string; settings?: unknown;
  };
  ...
  try {
    ...
    const rows = await prisma.$queryRaw<BranchRow[]>`
      ...
      VALUES (${branchId}, ${clinicId}, ${branchCode}, ${name.trim()}, ${city || null}, ${address || null}, ${phone || null}, true,
    ...
    if (error?.code === 'P2002' || String(error?.message || '').includes('branches_clinic_id_code_key')) {
...
branchesRouter.patch('/:id', async (req: AuthRequest, res) => {
  ...
  try {
    ...
    if (active === false && branch.is_default) {
      const activeDefaults = await prisma.$queryRaw<Array<{ count: bigint }>>`
        SELECT COUNT(*)::bigint AS count FROM "branches" WHERE "clinic_id" = ${branch.clinic_id} AND "active" = true
      `;
      if (Number(activeDefaults[0]?.count ?? 0) <= 1) {
        return res.status(409).json({ ok: false, error: 'Нельзя отключить единственный активный филиал' });
      }
    }
...
// POST /api/branches/:id/members/:userId — assign a clinic member to this branch.
branchesRouter.post('/:id/members/:userId', async (req: AuthRequest, res) => {
  const branchId = String(req.params.id);
  const userId = String(req.params.userId);
  try {
    const rows = await prisma.$queryRaw<Array<{ clinic_id: string; active: boolean }>>`
      SELECT "clinic_id", "active" FROM "branches" WHERE "id" = ${branchId} LIMIT 1
    `;
    const branch = rows[0];
    if (!branch) return res.status(404).json({ ok: false, error: 'Филиал не найден' });
    ...
    await prisma.$executeRaw`
      UPDATE "clinic_members" SET "branch_id" = ${branchId}, "updatedAt" = CURRENT_TIMESTAMP WHERE "userId" = ${userId} AND "clinicId" = ${branch.clinic_id}
    `;
