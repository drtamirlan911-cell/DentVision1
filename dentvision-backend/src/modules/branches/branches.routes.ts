...
branchesRouter.get('/:id/members', async (req: AuthRequest, res) => {
  ...
  try {
    ...
    if (!authz.allowed) return res.status(authz.status).json({ok:false,error:authz.error});
    const rows = await prisma.$queryRaw<Array<{id:string;user_id:string|null;full_name:string;email:string|null;person_type:string;active:boolean}>>`
      SELECT p."id", p."userId" AS user_id, p."fullName" AS full_name, COALESCE(u."email",p."email") AS email, p."personType" AS person_type, bm."active" AS active
    ...
    return res.json({ok:true,data:rows.map(r=>({id:r.id,userId:r.user_id,name:r.full_name,email:r.email,role:r.person_type,branchId,active:r.active}))});
