...
/** Public discovery — no patient data, only clinics accepting online booking. */
publicRouter.get('/clinics/discover', discoveryLimiter, async (req, res) => {
  try {
    ...
    const candidates = clinics
      ...
      .filter((clinic) => {
        if (!q) return true;
        return [clinic.name, clinic.city, clinic.address]
...
publicRouter.get('/clinic/:clinicId', async (req, res) => {
  try {
    ...
    const doctors = [
      ...members.map((m) => ({ id: m.user.id, name: [m.user.firstName, m.user.lastName].filter(Boolean).join(' ').trim(), spec: m.user.spec || undefined, avatar: m.user.avatar || undefined, phone: m.user.phone || undefined })),
      ...extraPersons.map((p) => ({ id: p.userId!, name: p.fullName, spec: p.specialization || undefined, avatar: p.avatar || undefined, phone: p.phone || undefined })),
    ];
    ...
    const services = priceRows.map((p) => {
      ...
      return { id: p.serviceCode, name, price: p.price, category: name.includes('·') ? name.split('·')[0].trim() : 'Услуги' };
    });
    return res.json({
      ok: true,
      clinic: { id: clinic.id, name: clinic.name, city: clinic.city, address: clinic.address, phone: clinic.phone, logo: clinic.logo },
      doctors, services,
      settings: { workStart: settings.workStart, workEnd: settings.workEnd, workDays: settings.workDays, lunchStart: settings.lunchStart, lunchEnd: settings.lunchEnd, bookingSlotMinutes: settings.bookingSlotMinutes, defaultAppointmentDuration: settings.defaultAppointmentDuration, currency: settings.currency },
    });
  } catch (error) {
...
publicRouter.post('/booking', publicBookingLimiter, async (req, res) => {
  try {
    ...
    if (settings.onlineBookingEnabled === false) return res.status(403).json({ ok: false, error: 'Онлайн-запись временно недоступна' });
    const day = new Date(`${dateStr}T12:00:00.000Z`);
    if (!isWorkingDay(day, settings)) return res.status(400).json({ ok: false, error: 'Клиника не работает в выбранный день' });
    ...
    if (doctorId) {
      const member = await prisma.clinicMember.findFirst({ where: { clinicId, userId: doctorId }, include: { user: { select: { firstName: true, lastName: true } } } });
      if (!member) return res.status(400).json({ ok: false, error: 'Врач не найден' });
...
publicRouter.get('/document/:token', documentSignLimiter, async (req, res) => {
  ...
  } catch (e) { res.status(500).json({ ok: false, error: 'Failed to load document' }); }
});
...
publicRouter.post('/document/:token/sign', documentSignLimiter, async (req, res) => {
  ...
  } catch (e: any) { res.status(e?.status || 500).json({ ok: false, error: e instanceof Error ? e.message : 'Failed to sign document' }); }
});
