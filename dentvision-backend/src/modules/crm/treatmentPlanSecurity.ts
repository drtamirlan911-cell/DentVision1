...
export async function assertTreatmentPlanDoctorInClinic(
  ...
  if (!member) {
    throw new TreatmentPlanReferenceError('DOCTOR_OUTSIDE_CLINIC');
  }
}
...
export async function assertTreatmentPlanReferencesInClinic(
  ...
  if (refs.appointmentId) {
    ...
    if (!appointment) {
      throw new TreatmentPlanReferenceError('APPOINTMENT_OUTSIDE_CLINIC');
    }
  }

  if (refs.invoiceId) {
    const invoice = await (prisma as any).invoice.findFirst({
      where: { id: refs.invoiceId, clinicId },
      select: { id: true },
    });
    if (!invoice) {
      throw new TreatmentPlanReferenceError('INVOICE_OUTSIDE_CLINIC');
    }
  }
}
