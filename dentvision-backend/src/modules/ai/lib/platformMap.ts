...
export function stageAwareSuggestions(opts: {
  ...
  const byStage: Record<string, string[]> = {
    schedule: clinical
      ? ['Кто сегодня у меня?', 'Открыть зубную карту', 'Создать план лечения']
      : ['Кто сегодня?', 'Записать пациента', 'Лист ожидания', 'Что важно сегодня?'],
    finance: clinical
      ? ['Показать расписание', 'Открыть зубную карту', 'Создать план лечения']
      : ['Проверить долги', 'Показать выручку', 'Открыть кассу'],
    inventory: clinical
      ? ['Показать расписание', 'Открыть зубную карту']
      : ['Что на складе', 'Открыть маркетплейс', 'Что заканчивается'],
    ...
    shop: clinical
      ? ['Показать расписание', 'Курсы для меня']
      : ['Подобрать расходники', 'Что на складе', 'Мои заказы'],
    school: ['Курсы для меня', 'Вебинары', 'Открыть кабинет лектора'],
    supplier: ['Мои товары', 'Заказы продавца'],
    analytics: clinical
      ? ['Показать расписание', 'Открыть зубную карту', 'Создать план лечения']
      : ['Показать выручку', 'Загрузка врачей', 'План загрузки клиники'],
    jobs: ['Найди вакансии', 'Разместить вакансию'],
    community: ['Открыть ленту', 'Сообщения'],
    marketing: clinical
      ? ['Показать расписание', 'Открыть зубную карту']
      : ['Активные акции', 'Открыть прайс'],
  ...
  // Clinical roles: role chips first so finance/analytics stages never crowd them out.
  return clinical
    ? [...new Set([...roleChips, ...stageChips])].slice(0, 4)
    : [...new Set([...stageChips, ...roleChips])].slice(0, 4)
}
