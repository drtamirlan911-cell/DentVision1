/**
 * Agent Registry — DentVision AI OS (Spec §15.18 + §16 personas)
 *
 * The single source of truth for which agents exist and what they are
 * allowed to do. An agent that is not registered here cannot be selected
 * by the orchestrator, and a tool not listed in `allowedTools` is never
 * exposed to that agent's LLM planning step.
 */

import type { PersonaId } from './persona.js';

export type AgentStatus = 'active' | 'beta' | 'disabled';

export type AgentDomain =
  | 'clinical'
  | 'business'
  | 'marketplace'
  | 'education'
  | 'compliance'
  | 'automation'
  | 'knowledge';

export interface AgentDefinition {
  /** `agent.<domain>.<name>` (Spec §15.18) */
  id: string;
  name: string;
  domain: AgentDomain;
  version: string;
  /** Clinic roles allowed to invoke this agent. '*' = any authenticated role. */
  requiredPermissions: string[];
  /** Tool names from os/tools.ts this agent may call. */
  allowedTools: string[];
  owner: string;
  status: AgentStatus;
  /** Injected into the system prompt when this agent leads the response. */
  mandate: string;
  /**
   * Operational persona tag (§16). Clinical specialty agents stay under Doctor
   * and do not get a top-level persona of their own.
   */
  persona?: PersonaId;
}

const AGENTS: AgentDefinition[] = [
  {
    id: 'agent.clinical.patient',
    name: 'Patient Agent',
    domain: 'clinical',
    version: '1.0.0',
    requiredPermissions: ['OWNER', 'ADMIN', 'DOCTOR', 'ASSISTANT', 'MANAGER', 'CASHIER', 'STUDENT', 'SUPPORT'],
    allowedTools: ['searchPatients', 'getPatientCard', 'getVisits', 'navigate'],
    owner: 'clinical-team',
    status: 'active',
    persona: 'doctor',
    mandate:
      'Ты работаешь с пациентами: поиск, медицинская карта, анамнез, история посещений. Не ставь диагнозы — окончательное клиническое решение принимает врач.',
  },
  {
    id: 'agent.clinical.treatment-planner',
    name: 'Treatment Planner',
    domain: 'clinical',
    version: '1.0.0',
    requiredPermissions: ['OWNER', 'DOCTOR'],
    allowedTools: ['searchPatients', 'getPatientCard', 'getTreatmentPlans', 'createTreatmentPlan', 'navigate'],
    owner: 'clinical-team',
    status: 'active',
    persona: 'doctor',
    mandate:
      'Ты составляешь планы лечения по одонтограмме: читай getPatientCard.odontogramSummary (статусы FDI + поверхности M/O/D/B/L, эндо ✓/✗, импланты). ' +
      'Предложи этапы с зубами и ориентиром бюджета, затем createTreatmentPlan (confirmed=false). План — черновик для врача.',
  },
  {
    id: 'agent.clinical.reception',
    name: 'Reception Agent',
    domain: 'clinical',
    version: '1.0.0',
    requiredPermissions: ['OWNER', 'ADMIN', 'DOCTOR', 'ASSISTANT', 'MANAGER', 'CASHIER'],
    allowedTools: [
      'getSchedule',
      'searchPatients',
      'getClinicLoadPlan',
      'getTreatmentPlans',
      'createAppointment',
      'updateAppointmentStatus',
      'cancelAppointment',
      'rescheduleAppointment',
      'navigate',
    ],
    owner: 'clinical-team',
    status: 'active',
    persona: 'reception',
    mandate:
      'Ты управляешь записью и загрузкой: при вопросах про пустые слоты / возврат пациентов / обзвон СРАЗУ вызывай getClinicLoadPlan и отвечай именами, телефонами и часами из данных. Без общих лекций.',
  },
  {
    id: 'agent.business.finance',
    name: 'Financial Agent',
    domain: 'business',
    version: '1.0.0',
    requiredPermissions: ['OWNER', 'ADMIN', 'MANAGER', 'CASHIER'],
    allowedTools: ['getRevenue', 'getDebtors', 'createInvoice', 'navigate'],
    owner: 'business-team',
    status: 'active',
    persona: 'finance',
    mandate:
      'Ты отвечаешь за финансы клиники: выручка, должники, счета. Показывай цифры точно, из данных, без выдумывания. Создание счёта требует подтверждения.',
  },
  {
    id: 'agent.business.analytics',
    name: 'Analytics Agent',
    domain: 'business',
    version: '1.0.0',
    requiredPermissions: ['OWNER', 'ADMIN', 'MANAGER'],
    allowedTools: ['getDashboardStats', 'getRevenue', 'getDoctorUtilization', 'getClinicLoadPlan', 'getDebtors', 'navigate'],
    owner: 'business-team',
    status: 'active',
    persona: 'analyst',
    mandate:
      'Ты делаешь аналитику и загрузку клиники. По загрузке/возврату базы/пустым слотам СРАЗУ getClinicLoadPlan — отвечай конкретными пациентами, телефонами, датами и часами. Не подменяй данные общей теорией и не заканчивай «если хотите, помогу».',
  },
  {
    id: 'agent.business.marketing',
    name: 'Marketing Agent',
    domain: 'business',
    version: '1.1.0',
    requiredPermissions: ['OWNER', 'ADMIN', 'MANAGER', 'DIRECTOR'],
    allowedTools: ['getPromotions', 'getRecallList', 'draftPromoCopy', 'getClinicLoadPlan', 'navigate'],
    owner: 'business-team',
    status: 'active',
    persona: 'marketing',
    mandate:
      'Ты — AI Marketing: акции, реактивация базы, тексты промо и WhatsApp-шаблоны (draft). ' +
      'Сначала данные: getPromotions / getRecallList. Не отправляй рассылки сам — только черновики и списки. Навигация в Акции / Прайс.',
  },
  {
    id: 'agent.business.ceo',
    name: 'CEO Agent',
    domain: 'business',
    version: '1.1.0',
    requiredPermissions: ['OWNER', 'DIRECTOR', 'ADMIN', 'MANAGER'],
    allowedTools: [
      'composeCeoBrief',
      'getDashboardStats',
      'getRevenue',
      'getDebtors',
      'getClinicLoadPlan',
      'getPromotions',
      'getRecallList',
      'navigate',
    ],
    owner: 'business-team',
    status: 'active',
    persona: 'ceo',
    mandate:
      'Ты — AI CEO: синтезируешь Analyst + Finance + Marketing. Не пишешь SQL и не обходишь RBAC. ' +
      'Для «что важно / приоритеты недели» вызывай composeCeoBrief. Делегируй детали: долги→Finance, recall→Marketing, слоты→Reception.',
  },
  {
    id: 'agent.marketplace.shop',
    name: 'Shop Agent',
    domain: 'marketplace',
    version: '1.0.0',
    requiredPermissions: ['*'],
    allowedTools: ['searchProducts', 'navigate'],
    owner: 'marketplace-team',
    status: 'active',
    persona: 'supply',
    mandate:
      'Ты помогаешь с закупками: поиск и подбор товаров в маркетплейсе. Заказы оформляет пользователь через корзину.',
  },
  {
    id: 'agent.marketplace.inventory',
    name: 'Inventory Agent',
    domain: 'marketplace',
    version: '1.0.0',
    requiredPermissions: ['OWNER', 'ADMIN', 'DOCTOR', 'ASSISTANT', 'MANAGER', 'CASHIER'],
    allowedTools: ['getInventory', 'searchProducts', 'navigate'],
    owner: 'marketplace-team',
    status: 'active',
    persona: 'supply',
    mandate:
      'Ты следишь за складом клиники: остатки, что заканчивается, что пора дозаказать (с подбором из маркетплейса).',
  },
  {
    id: 'agent.education.school',
    name: 'School Agent',
    domain: 'education',
    version: '1.0.0',
    requiredPermissions: ['*'],
    allowedTools: ['searchCourses', 'navigate'],
    owner: 'education-team',
    status: 'active',
    persona: 'education',
    mandate:
      'Ты подбираешь обучение: курсы Академии по специализации и уровню. Рекомендуй конкретные курсы с обоснованием.',
  },
  {
    id: 'agent.knowledge.guest',
    name: 'Guest Concierge',
    domain: 'knowledge',
    version: '1.0.0',
    requiredPermissions: ['GUEST'],
    allowedTools: ['searchCourses', 'searchProducts', 'navigate'],
    owner: 'growth-team',
    status: 'active',
    persona: 'guest',
    mandate:
      'Ты — дружелюбный гид DentVision для гостя. Объясняй платформу простым языком: CRM клиники, маркетплейс, Academy OS, ИИ-ассистент. Не выдумывай данные клиники (расписание, выручку, долги) — у гостя нет клиники. Предлагай демо, регистрацию, маркетплейс и академию. Отвечай как чистый чат-ассистент, без канцелярита. Названия разделов пиши только по-русски, без английских ключей вроде schedule/patients.',
  },
  {
    id: 'agent.clinical.dental',
    name: 'Dental Agent',
    domain: 'clinical',
    version: '1.0.0',
    requiredPermissions: ['OWNER', 'ADMIN', 'DOCTOR', 'ASSISTANT', 'MANAGER'],
    allowedTools: ['searchPatients', 'getPatientCard', 'getVisits', 'searchClinicalNotes', 'applyToothFindings', 'getTreatmentPlans', 'createTreatmentPlan', 'navigate'],
    owner: 'clinical-team',
    status: 'active',
    persona: 'doctor',
    mandate: 'Общий стоматологический контур: карта пациента, клинические записи, одонтограмма и черновик плана лечения. Не подтверждай клинические решения за врача.',
  },
  {
    id: 'agent.clinical.radiology',
    name: 'Radiology Agent',
    domain: 'clinical',
    version: '1.0.0',
    requiredPermissions: ['OWNER', 'ADMIN', 'DOCTOR'],
    allowedTools: ['getPatientCard', 'searchClinicalNotes', 'analyzeRadiograph', 'radiograph_findings', 'navigate'],
    owner: 'clinical-team',
    status: 'active',
    persona: 'doctor',
    mandate: 'Радиологический контур: анализ доступных изображений и структурирование находок. Любой результат — предварительный материал для врача, не окончательный диагноз.',
  },
  {
    id: 'agent.clinical.endodontic',
    name: 'Endodontic Agent',
    domain: 'clinical',
    version: '1.0.0',
    requiredPermissions: ['OWNER', 'DOCTOR'],
    allowedTools: ['getPatientCard', 'getVisits', 'searchClinicalNotes', 'applyToothFindings', 'getTreatmentPlans', 'createTreatmentPlan', 'navigate'],
    owner: 'clinical-team',
    status: 'active',
    persona: 'doctor',
    mandate: 'Эндодонтический контур: анализ записей и одонтограммы, подготовка черновика эндодонтического плана. Решение остаётся за врачом.',
  },
  {
    id: 'agent.clinical.orthopedic',
    name: 'Orthopedic Agent',
    domain: 'clinical',
    version: '1.0.0',
    requiredPermissions: ['OWNER', 'DOCTOR'],
    allowedTools: ['getPatientCard', 'getVisits', 'getTreatmentPlans', 'createTreatmentPlan', 'applyToothFindings', 'navigate'],
    owner: 'clinical-team',
    status: 'active',
    persona: 'doctor',
    mandate: 'Ортопедический контур: восстановление зубов, коронки, виниры и этапность лечения на основании подтверждённых данных карты.',
  },
  {
    id: 'agent.clinical.orthodontic',
    name: 'Orthodontic Agent',
    domain: 'clinical',
    version: '1.0.0',
    requiredPermissions: ['OWNER', 'DOCTOR'],
    allowedTools: ['getPatientCard', 'getVisits', 'getTreatmentPlans', 'createTreatmentPlan', 'navigate'],
    owner: 'clinical-team',
    status: 'active',
    persona: 'doctor',
    mandate: 'Ортодонтический контур: подготовка предварительного плана по данным пациента. Не выдавай окончательные клинические рекомендации без врача.',
  },
  {
    id: 'agent.clinical.lab',
    name: 'Laboratory Agent',
    domain: 'clinical',
    version: '1.0.0',
    requiredPermissions: ['OWNER', 'ADMIN', 'DOCTOR', 'LAB', 'SUPPORT'],
    allowedTools: ['getLabOrders', 'createLabOrder', 'updateLabOrderStatus', 'createDiagnosticReferral', 'navigate'],
    owner: 'clinical-team',
    status: 'active',
    persona: 'doctor',
    mandate:
      'Ты следишь за лабораторными заказами и направлениями на диагностику: статусы, сроки, просрочки. ' +
      'Новый заказ или направление — сначала createLabOrder/createDiagnosticReferral (confirmed=false), это черновик для подтверждения врачом.',
  },
  {
    id: 'agent.marketplace.buyer',
    name: 'Buyer Agent',
    domain: 'marketplace',
    version: '1.0.0',
    requiredPermissions: ['BUYER', 'OWNER', 'ADMIN', 'MANAGER'],
    allowedTools: ['getInventory', 'searchProducts', 'navigate'],
    owner: 'marketplace-team',
    status: 'active',
    persona: 'supply',
    mandate:
      'Ты закупщик: склад клиники → аналоги в маркетплейсе → заказы. Без клинических диагнозов.',
  },
  {
    id: 'agent.marketplace.supplier',
    name: 'Supplier Agent',
    domain: 'marketplace',
    version: '1.0.0',
    requiredPermissions: ['SUPPLIER'],
    allowedTools: ['searchProducts', 'navigate'],
    owner: 'marketplace-team',
    status: 'active',
    persona: 'supply',
    mandate:
      'Ты гид кабинета продавца: товары, заказы маркетплейса. CRM чужой клиники не трогай.',
  },
  {
    id: 'agent.education.lecturer',
    name: 'Lecturer Agent',
    domain: 'education',
    version: '1.0.0',
    requiredPermissions: ['LECTURER'],
    allowedTools: ['searchCourses', 'navigate'],
    owner: 'education-team',
    status: 'active',
    persona: 'education',
    mandate:
      'Ты помогаешь лектору: курсы, вебинары, кабинет лектора Academy OS.',
  },
  {
    id: 'agent.partner.diagnostics',
    name: 'Diagnostics Workspace Agent',
    domain: 'business',
    version: '1.0.0',
    requiredPermissions: [
      'DIAGNOSTIC_OWNER',
      'DIAGNOSTIC_ADMIN',
      'DIAGNOSTIC_MANAGER',
      'DIAGNOSTIC_OPERATOR',
      'DIAGNOSTIC_RECEPTION',
      'DIAGNOSTIC_FINANCE',
      'DIAGNOSTIC_QUALITY',
      'MEDICAL_LAB_OWNER',
      'MEDICAL_LAB_ADMIN',
      'MEDICAL_LAB_MANAGER',
      'MEDICAL_LAB_RECEPTION',
      'MEDICAL_LAB_TECHNICIAN',
      'DENTAL_LAB_OWNER',
      'DENTAL_LAB_ADMIN',
      'DENTAL_LAB_MANAGER',
      'DENTAL_TECHNICIAN',
    ],
    allowedTools: ['navigate', 'searchProducts', 'searchCourses'],
    owner: 'partner-team',
    status: 'active',
    persona: 'analyst',
    mandate:
      'Ты работаешь внутри текущего партнёрского рабочего пространства. Не выдавай его за клинику и не используй клинические данные другой организации. Веди пользователя по доступным функциям текущего кабинета.',
  },
  {
    id: 'agent.platform.superadmin',
    name: 'Platform Admin Agent',
    domain: 'compliance',
    version: '1.0.0',
    requiredPermissions: ['SUPERADMIN'],
    allowedTools: ['navigate', 'getDashboardStats'],
    owner: 'platform-team',
    status: 'active',
    mandate:
      'Ты суперадмин платформы: клиники, аудит, бэкапы. Веди через navigate в /admin, /audit, /backup.',
  },
];

export function listAgents(): AgentDefinition[] {
  return AGENTS.filter((a) => a.status !== 'disabled');
}

export function agentsForRole(role: string): AgentDefinition[] {
  const normalized = role.toUpperCase();
  return listAgents().filter(
    (a) => a.requiredPermissions.includes('*') || a.requiredPermissions.includes(normalized),
  );
}

/** Agents tagged with an operational persona (§16). */
export function agentsForPersona(persona: PersonaId, role?: string): AgentDefinition[] {
  const pool = role ? agentsForRole(role) : listAgents();
  return pool.filter((a) => a.persona === persona);
}

/** Union of tools available to a role across all its permitted agents. */
export function toolsForRole(role: string): Set<string> {
  const tools = new Set<string>();
  for (const agent of agentsForRole(role)) {
    for (const tool of agent.allowedTools) tools.add(tool);
  }
  return tools;
}
