...
function notificationId(eventId: string, userId: string, actionIndex: number): string {
  // Notification.id is the only durable uniqueness primitive currently available.
  // Deriving it from event/user/action makes redelivery idempotent without a schema
  // migration, while the digest is short enough for the existing VARCHAR(50).
  return `ai_${createHash('sha256').update(`${eventId}:${userId}:${actionIndex}`).digest('hex').slice(0, 45)}`;
}
