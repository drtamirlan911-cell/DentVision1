...
// Self-service academy onboarding. Creating an academy never requires a
// DentVision operator; verification and legal activation remain asynchronous.
academiesRouter.post('/register', async (req: AuthRequest, res) => {
  try {
    const { name, city, description, legalName, bin, address, phone, email } = req.body || {};
    const academyName = String(name || '').trim();
    if (!academyName) {
      return res.status(400).json({ ok: false, error: 'Название академии обязательно' } satisfies ApiResponse);
    }

    const existing = await prisma.academy.findFirst({
      where: { ownerId: req.user!.id, name: academyName },
    });
    if (existing) {
      const organization = await prisma.organization.findUnique({
        where: { originalType_originalId: { originalType: 'Academy', originalId: existing.id } },
      });
      return res.status(409).json({
        ok: false,
        error: 'Вы уже зарегистрировали эту академию.',
        data: { academy: existing, organizationId: organization?.id },
    ...
    // Unified organization membership for the owner. This gives the new
    // academy an immediate workspace context without a separate membership table.
    await prisma.person.upsert({
      ...
      update: {
        ...
        contacts: { role: 'OWNER', academyId: academy.id } as Record<string, unknown>,
      },
      create: {
        id: uid(),
        fullName: [req.user!.firstName, req.user!.lastName].filter(Boolean).join(' ') || academy.name,
        ...
        originalId: `${academy.id}:${req.user!.id}`,
        contacts: { role: 'OWNER', academyId: academy.id } as Record<string, unknown>,
      },
    });
    ...
    const legal = await ensureLegalTrustPackage({
      ...
      organizationId: organization.id,
      type: 'ACADEMY',
      legalName: String(legalName || academyName),
      bin: bin || null,
      director: [req.user!.firstName, req.user!.lastName].filter(Boolean).join(' ') || null,
...
academiesRouter.patch('/:id', requirePermission('academy.manage'), async (req: AuthRequest, res) => {
  try {
    ...
    await prisma.organization.upsert({
      ...
      create: { id: uid(), name: updated.name, type: 'ACADEMY', originalType: 'Academy', originalId: updated.id },
    });
  ...
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.lecturer.findMany({
        ...
        include: { academy: { select: { id: true, name: true } }, user: { select: { firstName: true, lastName: true, email: true } }, verifications: true, _count: { select: { courses: true } } },
      }),
...
lecturersRouter.get('/:id', async (req: AuthRequest, res) => {
  try {
    const lecturer = await prisma.lecturer.findUnique({
      ...
      include: { verifications: true, academy: { select: { id: true, name: true } }, user: { select: { firstName: true, lastName: true, email: true } } },
    });
