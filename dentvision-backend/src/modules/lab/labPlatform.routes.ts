...
const STATUS_FLOW: Record<string, string[]> = {
  ...
  sent: ['in_progress', 'delayed', 'cancelled'],
  in_progress: ['try_in', 'adjustment', 'ready', 'remake', 'delayed', 'cancelled'],
  try_in: ['adjustment', 'ready', 'remake'],
  adjustment: ['ready', 'remake'],
  ready: ['delivered', 'remake'],
  delivered: [],
  remake: ['in_progress', 'delayed', 'cancelled'],
...
async function resolveLab(req: AuthRequest) {
  ...
  const membership = await prisma.laboratoryMember.findFirst({
    where: { labId: user.organizationId, userId: user.id },
    include: { lab: true },
  });
...
async function ordersForLab(labId: string, status?: string) {
  const rows = await prisma.labOrder.findMany({
    where: {
      ...
      files: { path: ['meta', 'laboratoryId'], equals: labId },
    },
    include: {
...
labPlatformRouter.get('/dashboard', async (req: AuthRequest, res) => {
  try {
    ...
    console.error('[LabPlatform] dashboard error:', error);
    return res.status(500).json({ ok: false, error: 'Не удалось загрузить кабинет лаборатории' } satisfies ApiResponse);
  }
});
...
labPlatformRouter.get('/orders', async (req: AuthRequest, res) => {
  try {
    ...
    const orders = await ordersForLab(membership.labId, status);
    return res.json({ ok: true, data: orders } satisfies ApiResponse);
  } catch (error) {
    console.error('[LabPlatform] orders error:', error);
    return res.status(500).json({ ok: false, error: 'Не удалось загрузить заказы лаборатории' } satisfies ApiResponse);
  }
});
...
labPlatformRouter.patch('/orders/:id/status', async (req: AuthRequest, res) => {
  try {
    ...
    const existing = await prisma.labOrder.findFirst({
      where: { id: req.params.id, files: { path: ['meta', 'laboratoryId'], equals: membership.labId } },
    });
    if (!existing) return res.status(404).json({ ok: false, error: 'Заказ не найден в этой лаборатории' } satisfies ApiResponse);
    if (!STATUS_FLOW[existing.status]?.includes(next)) {
      return res.status(409).json({ ok: false, error: `Переход ${existing.status} → ${next} недоступен` } satisfies ApiResponse);
    }
    ...
    publish('labOrder.status_changed', {
      ...
      laboratoryId: membership.labId,
      userId: req.user?.id,
    });
    return res.json({ ok: true, data: serialize(order) } satisfies ApiResponse);
  } catch (error) {
    console.error('[LabPlatform] status error:', error);
    return res.status(500).json({ ok: false, error: 'Не удалось изменить статус заказа' } satisfies ApiResponse);
  }
});
...
labPlatformRouter.patch('/orders/:id/technician', async (req: AuthRequest, res) => {
  try {
    ...
    console.error('[LabPlatform] technician error:', error);
    return res.status(500).json({ ok: false, error: 'Не удалось назначить техника' } satisfies ApiResponse);
  }
});
...
labPlatformRouter.get('/team', async (req: AuthRequest, res) => {
  try {
    ...
    console.error('[LabPlatform] team error:', error);
    return res.status(500).json({ ok: false, error: 'Не удалось загрузить команду' } satisfies ApiResponse);
  }
});

// Clinic-side assignment. This keeps LabOrder canonical while recording the real laboratory target.
labPlatformRouter.post('/assign', async (req: AuthRequest, res) => {
  try {
    ...
    const laboratoryId = String(req.body?.laboratoryId || '');
    ...
    console.error('[LabPlatform] assign error:', error);
    return res.status(500).json({ ok: false, error: 'Не удалось передать заказ лаборатории' } satisfies ApiResponse);
  }
});
