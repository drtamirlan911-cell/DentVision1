...
prismaWithReferralEconomics.$use?.(async (params, next) => {
  ...
  // TreatmentCase inheritance is database-enforced by the migration trigger.
  // Keeping it below the Prisma middleware avoids a second write outside an
  // interactive transaction and guarantees the same behavior for every writer.
  return next(params);
});
