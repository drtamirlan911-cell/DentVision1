...
class PublicRepository(private val api: ApiClient = ServiceLocator.api) {
    ...
    suspend fun products(search: String? = null, category: String? = null): List<ShopProduct> =
        apiCall { api.public.products(search = search?.ifBlank { null }, category = category?.ifBlank { null }) }

    suspend fun categories(): List<ShopCategory> =
        apiCall { api.public.categories() }
