...
import kz.dentvision.crm.lib.exportCsv
...
fun FinanceScreen(viewModel: FinanceViewModel = viewModel()) {
    ...
    Column(
        ...
        Row(
            ...
            val report = (state.report as? UiState.Data)?.value
            IconButton(
                enabled = report != null,
                onClick = {
                    if (report != null) {
                        ...
                        exportCsv(
                            context = context,
                            filename = "finance-${report.from ?: "period"}_${report.to ?: ""}.csv",
                            headers = listOf("Метрика", "Значение"),
                            rows = listOf(
                                listOf("Выручка", totals.revenue),
                                listOf("Расходы", totals.expenses),
                                listOf("ФОТ", totals.payroll),
                                listOf("Прибыль", totals.profit),
                                listOf("Долги", totals.unpaid),
                            ),
                        )
                    }
                },
                ...
                Icon(
                    ...
                    tint = if (report != null) DvTheme.colors.gold else DvTheme.colors.textMuted,
                )
            }
        }
