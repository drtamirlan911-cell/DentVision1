...
export const TOOTH_16: ToothDefinition = {
  ...
  knownLimitations: [
    "crown.outline='rhomboid' is now read by crownGeometry.ts (`outlineFactor`), pulling the wall/table footprint toward a rhomboid instead of a plain ellipse — a real, visible fix triggered directly by comparing a live render against a standard 6-view dental-atlas reference plate the reviewer supplied, where the occlusal panel showed a clear diamond/kite shape against our near-perfect circle. Built from two cosine harmonics (4-fold for the corner/side contrast, 2-fold to differentiate the acute mesiobuccal/distolingual corners from the obtuse mesiolingual/distobuccal ones — see references). Confirmed numerically (occlusal-edge ring radius: acute corners ≈5.37mm, obtuse corners ≈4.58mm, flat sides ≈3.65-4.00mm — a real, ordered contrast, not noise) and visually (re-rendered occlusal view clearly reads as a rounded rhomboid, not a circle). Two honest gaps versus the reference: (1) the modulation strength was tuned to stay safely inside the existing bbox validation tolerance (`[0.6x, 1.05x]` of the defined crown width) rather than to match the reference plate's corner sharpness exactly — the reference's corners are noticeably crisper/more pointed than this model's, which uses a smooth cosine (not a true polygon-with-rounded-corners) and reads as a rounded square more than a sharp diamond; (2) the same angle-only factor is applied uniformly at every height (cervical line to occlusal edge) for simplicity, whereas a real tooth's cross-section is probably more rhomboidal near the occlusal table and more rounded near the cervical line — not modelled.",
