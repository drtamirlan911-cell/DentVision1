...
export const TOOTH_16: ToothDefinition = {
  ...
  roots: [
    ...
    },
    {
      ...
      baseDiameterMm: 4.5,
      apexDiameterMm: 1.4,
      // Diverges lingually/palatally — MB<->palatal / DB<->palatal are the widest
      // divergences. -32.9 makes the DB<->palatal angle (12 - (-32.9) = 44.9deg)
      // match the CBCT-cited mean exactly (PMC8665178) — a prior pass set this to
      // -28deg (giving 40deg, not 44.9deg) while the knownLimitations entry already
      // (incorrectly) claimed the 44.9deg figure; corrected data to match the citation
      // rather than leaving the honesty-discipline mismatch in place.
      buccolingualTiltDeg: -32.9,
  ...
  references: [
    ...
    // Added while matching a standard 6-view dental-atlas reference plate
    // supplied directly by the reviewer (occlusal panel showed a clear
    // rhomboidal/diamond outline, not the near-circle crownGeometry.ts was
    // producing — see knownLimitations and Bugs found and fixed for the fix).
    {
      source: 'Maxillary first molar occlusal outline — rhomboidal shape, acute/obtuse line angles (aggregated dental-anatomy sources, incl. ScienceDirect Topics summary)',
      type: 'glossary_crossref',
      snippetOnly: true,
      confirmedFeatures: [
        'occlusal outline is rhomboidal, wider buccolingually than mesiodistally',
        'mesiobuccal and distolingual line angles are acute (sharper, more prominent corners)',
        'mesiolingual and distobuccal line angles are obtuse (flatter, less prominent corners)',
      ],
    },
    // Added when the reviewer directly asked why the outline was a tuned
    // formula instead of being traced from the picture they supplied —
    // fair question. This is that trace: 8 radius samples read off the
    // occlusal panel via a polar-grid overlay (see engine/crownGeometry.ts
    // `TRACED_OUTLINE_SAMPLES` and docs/DENTAL_3D_ENGINE.md for the method
    // and raw readings). Distinct reference type on purpose — this is a
    // hand-digitized illustration, not a cited measurement; see
    // `reviewer_reference_image` in anatomy/types.ts.
    {
      source: 'Reviewer-supplied dental-atlas 6-view reference plate (occlusal panel), traced via polar-grid overlay',
      type: 'reviewer_reference_image',
      confirmedFeatures: [
        'occlusal outline is not 4-fold symmetric: buccal/lingual sides read more recessed than mesial/distal sides',
        'one corner (largest cusp, mesiolingual) reads clearly more prominent than the other three — not a clean 2-acute/2-obtuse split',
        'mesial/distal panels: roots read as fairly short/stubby, not more dramatically diverging than the model already produced',
      ],
    },
  ],

  knownLimitations: [
    "crown.outline='rhomboid' is now read by crownGeometry.ts (`outlineFactor`), pulling the wall/table footprint toward a rhomboid instead of a plain ellipse — a real, visible fix triggered directly by comparing a live render against a standard 6-view dental-atlas reference plate the reviewer supplied, where the occlusal panel showed a clear diamond/kite shape against our near-perfect circle. First pass used two cosine harmonics tuned against the bbox validation tolerance, not against the reference's actual shape — the reviewer directly asked why that wasn't just traced from the picture, which was a fair objection: a hand-picked formula is an indirect stand-in for a shape that's sitting right in the reference. Replaced with `TRACED_OUTLINE_SAMPLES`, 8 radius values read directly off the reference's occlusal panel via a polar-grid overlay (0/45/90/.../315°, 3 concentric reference circles), smoothstep-interpolated between samples. This surfaced a real finding a symmetric formula couldn't have: the traced outline isn't 4-fold symmetric the way the cosine version assumed — the buccal/lingual sides read as more recessed than the mesial/distal sides, and one corner (mesiolingual, the largest cusp) reads clearly more prominent than the other three, not a clean 2-acute/2-obtuse split. Confirmed numerically (occlusal-edge ring radius now: mesiolingual ≈4.91mm, mesiobuccal ≈4.82mm, distolingual ≈4.60mm, distobuccal ≈4.33mm, mesial ≈4.26mm, distal ≈4.13mm, buccal/lingual ≈4.06mm each — bbox comfortably inside the `[0.6x, 1.05x]` tolerance) and visually (re-rendered occlusal view shows the same asymmetric-corner character as the reference, not a uniform rounded square). Honest caveats, unavoidable for a hand-traced illustration: the 8 samples were read by eye against an overlaid grid, not machine-measured — real measurement noise, not sub-millimeter precision; the reference is a stylized rendering, not a laser scan of tooth 16 specifically; and the traced factor is still applied uniformly at every wall height (cervical line to occlusal edge) rather than varying the rhomboid's prominence with height, which a real cross-section probably does.",
    ...
    'Direct user feedback — "the occlusal part looks concave, like it\'s sunken in, but it should have relief" — led to a real, confirmed, and now-fixed bug: `occlusalRelief` combined cusps/ridges via `smoothMaxCompact` (deepest/tallest feature wins) but combined grooves and fossae via plain SUBTRACTION, summing their depths whenever they overlapped. Sampled numerically at the central fossa\'s own defined center: naive addition gave -2.283mm total depth versus the fossa\'s own defined 1.15mm depth — nearly double — because the central developmental groove, a second groove\'s edge, and the central fossa all converge within ~0.3-0.8mm of that point (by design, per the fossa/groove path comments above). Fixed by combining groove/fossa depth magnitudes via the same `smoothMaxCompact` pattern already used for cusps/ridges (deepest depression wins, blended smoothly at overlaps) before subtracting. Confirmed numerically (the deepest point in the sampled cross-sections rose from Y=3.979mm to Y=4.641mm — a real, measured reduction) and visually (re-rendered occlusal view). This is a real, meaningful reduction in over-deepening, but does NOT by itself resolve the larger, already-documented item above (occlusal view still reading as one basin dominated by the groove/fossa system rather than 4 clearly differentiated raised mounds) — that redesign (wider cusp tangential reach or reworked central-groove parameters) remains open.',
    ...
    'Root divergence angles were tuned to approximate the literature mean (DB-palatal ≈44.9°) rather than reproducing one measured specimen — this entry itself was, until the same reference-plate comparison that fixed the occlusal outline (above), inaccurate: the palatal root\'s tilt actually produced 40° (12 - (-28)), not the claimed 44.9°. Corrected the tilt (-28°→-32.9°) to match the cited figure exactly rather than leaving the honesty-discipline mismatch between this text and the data in place. MB-palatal divergence (currently ≈41.9°) is not itself individually cited — only the "MB/DB↔palatal are the widest divergences" relationship is documented, not a specific MB-palatal number — so it was left as a byproduct of the DB-palatal correction, not independently tuned to any further target (including the wider splay a generic/stylized dental-atlas illustration can show, which may not reflect the CBCT-measured mean). A later direct trace of the reviewer\'s reference plate\'s mesial/distal panels (see the outline-tracing entry above) found the roots there actually read as fairly short/stubby, not more dramatically splayed than this model — so no further widening was applied; the earlier caveat about a stylized illustration overstating divergence did not end up applying in practice for this specific reference.',
