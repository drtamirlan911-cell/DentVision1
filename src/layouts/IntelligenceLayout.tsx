...
export const IntelligenceLayout: React.FC = () => {
  ...
  const {
    sidebarOpen, toggleSidebar, contextSheetOpen, setContextSheetOpen,
    sidebarCollapsed, setSidebarCollapsed, sidebarPinned, sidebarVisible,
    setSidebarVisible, sidebarHovering, setSidebarHovering, firstRunPhase,
    setFirstRunPhase, completeFirstRun, toggleSidebarCollapsed, crumbTailLabel,
  } = useUIStore();
  ...
  const BREADCRUMB_LABELS: Record<string, string> = {
    crm: 'CRM', schedule: t('nav.schedule'), patients: t('nav.patients'),
    'medical-card': t('nav.medical_card'), finance: t('nav.finance'),
    cashier: t('nav.cashier'), inventory: t('nav.inventory'), documents: t('nav.documents'),
    'dental-chart': t('nav.dental_chart'), 'treatment-plans': t('nav.treatment_plans'),
    lab: t('nav.lab'), pricelist: t('nav.pricelist'), staff: t('nav.staff'),
    reminders: t('nav.reminders'), promotions: t('nav.promotions'), icd10: t('nav.icd10'),
    visits: t('nav.visits'), profile: t('nav.profile'), 'clinic-settings': t('nav.clinic_settings'),
    billing: t('nav.billing'), settings: t('nav.settings'), admin: t('nav.administration'),
    audit: t('nav.audit'), backup: t('nav.backup'), supplier: t('nav.supplier_cabinet'),
    shop: t('nav.shop'), school: 'Academy OS', 'school-workspace': t('nav.school_workspace'),
    'center-workspace': t('nav.center_workspace'), analytics: t('nav.analytics'),
    jobs: t('nav.jobs'), community: t('nav.community'),
  };
  ...
  useEffect(() => {
    if (!isGuest) return;
    setSidebarVisible(true); setSidebarCollapsed(false); setFirstRunPhase('done');
    completeFirstRun(); setOnboardingComplete(true); firstRunBooted.current = true;
  ...
  useEffect(() => {
    ...
    if (!isAIHome) {
      setSidebarVisible(true); setFirstRunPhase('done'); completeFirstRun();
      setOnboardingComplete(true); return;
    }
    firstRunBooted.current = true;
    trackProductEvent('first_run_started', { role: user?.role || 'guest' });
    setFirstRunPhase('greeting'); setSidebarVisible(false); setSidebarCollapsed(false);
    const dockTimer = setTimeout(() => {
      setFirstRunPhase('docking'); setSidebarVisible(true); setFirstRunPhase('docked');
  ...
  useEffect(() => {
    ...
    collapseTimerRef.current = setTimeout(() => {
      setSidebarCollapsed(true); setFirstRunPhase('collapsed'); completeFirstRun();
      setOnboardingComplete(true); trackProductEvent('sidebar_auto_collapsed', { after_ms: FIRST_RUN_COLLAPSE_MS });
        ...
        return (
          <div className="fixed inset-0 z-50 bg-surface-0 overflow-hidden flex items-center justify-center">
            <div className="text-center space-y-4">
              <div className="mx-auto w-16 h-16 rounded-2xl bg-dv-gold/15 flex items-center justify-center"><Stethoscope size={24} className="text-dv-gold" /></div>
    ...
    <div className="fixed inset-0 z-50 bg-surface-0 overflow-hidden flex">
      ...
      <div onMouseEnter={() => setSidebarHovering(true)} onMouseLeave={() => setSidebarHovering(false)} className="contents">
        <Sidebar
          ...
          roleInfo={isGuest ? { label: t('nav.guest'), icon: User, pages: ['shop', 'school', 'jobs', 'community', 'demo'] } as any : roleInfo}
      ...
      {!isMobile && (
        <AnimatePresence>
          {contextSheetOpen && (
            <motion.aside initial={{ width: 0, opacity: 1 }} animate={{ width: 320, opacity: 1 }} exit={{ width: 0, opacity: 0 }} transition={{ type: 'spring', stiffness: 350, damping: 30 }} className="hidden lg:flex flex-col border-l border-bdr-subtle bg-surface-1 overflow-hidden flex-shrink-0 h-full">
