...
import { Activity, BarChart3, BriefcaseBusiness, CalendarDays, ChevronRight, CircleHelp, Command, Database, FileCheck2, FileText, GraduationCap, LayoutDashboard, LogOut, MessageCircle, Package, Settings, ShieldCheck, ShoppingBag, Sparkles, Stethoscope, UserRound, Users, WalletCards, X, ClipboardList, Bot, Building2, FlaskConical, HeartPulse } from 'lucide-react';
...
function EcosystemSidebar({ collapsed, setCollapsed, sidebarVisible, isMobile, sidebarOpen, user, logout, toggleSidebar, isGuest = false, pendingApprovals = 0, isAdmin = false }: EcosystemSidebarProps) {
  ...
  return <>
    ...
    <motion.aside initial={false} animate={{ width, x: isMobile && !sidebarOpen ? -300 : 0 }} transition={{ type: 'spring', stiffness: 360, damping: 34 }} className="fixed inset-y-0 left-0 z-50 flex flex-col overflow-hidden border-r border-[var(--dv-border)] bg-[var(--dv-sidebar)]">
      ...
      <div className="border-t border-[var(--dv-border)] p-3"><div className={cn('flex items-center gap-2.5 rounded-xl p-2',collapsed&&!isMobile&&'justify-center')}><Avatar src={(user as any)?.avatar} name={(user as any)?.name || (user as any)?.firstName || 'User'} size="sm"/>{(!collapsed||isMobile)&&<div className="min-w-0 flex-1"><div className="truncate text-xs font-semibold">{(user as any)?.name || [(user as any)?.firstName,(user as any)?.lastName].filter(Boolean).join(' ') || 'DentVision'}</div><div className="truncate text-[10px] text-[var(--dv-muted)]">{(user as any)?.platformRole || (user as any)?.role || 'Участник экосистемы'}</div></div>}{!isGuest&&(!collapsed||isMobile)&&<button type="button" onClick={logout} aria-label="Выйти" className="rounded-lg p-2 text-[var(--dv-muted)] hover:bg-[var(--dv-nav-hover)]"><LogOut size={15}/></button>}</div></div>
    </motion.aside>
  </>;
}
