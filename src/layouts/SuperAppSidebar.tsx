...
function NavItem({ item, collapsed, active, onNavigate, label }: { item: Item; collapsed: boolean; active: boolean; onNavigate: (path: string) => void; label: string }) {
  return <button type="button" aria-current={active ? 'page' : undefined} title={collapsed ? label : undefined} onClick={() => onNavigate(item.path)} className={cn('group relative flex w-full items-center gap-3 rounded-[11px] px-2.5 py-2 text-left transition-all duration-200', collapsed ? 'justify-center px-2' : '', active ? 'bg-[var(--dv-nav-active)] text-[var(--dv-text)]' : 'text-[var(--dv-muted)] hover:bg-[var(--dv-nav-hover)] hover:text-[var(--dv-text)]')}>
    ...
    <span className={cn('grid h-8 w-8 shrink-0 place-items-center rounded-[9px] transition-colors', active ? 'bg-[var(--dv-accent-soft)] text-[var(--dv-accent)]' : 'bg-[var(--dv-icon-bg)] text-current')}>{React.cloneElement(item.icon, { size: 17, strokeWidth: 1.8 })}</span>
