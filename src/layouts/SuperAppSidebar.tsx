...
export interface SuperAppSidebarProps { collapsed: boolean; setCollapsed: (value: boolean) => void; sidebarVisible: boolean; isMobile: boolean; sidebarOpen: boolean; user: UserType | null; logout: () => void; toggleSidebar: () => void; isGuest?: boolean; pendingApprovals?: number; isAdmin?: boolean; }
...
function isActive(pathname: string, itemPath: string) { if (itemPath === '/ai') return pathname === '/ai' || pathname === '/'; return pathname === itemPath || pathname.startsWith(`${itemPath}/`); }
function NavItem({ item, collapsed, active, onNavigate, label }: { item: Item; collapsed: boolean; active: boolean; onNavigate: (path: string) => void; label: string }) {
  return <button type="button" aria-current={active ? 'page' : undefined} title={collapsed ? label : undefined} onClick={() => onNavigate(item.path)} className={cn('group relative flex w-full items-center gap-3 rounded-[11px] px-2.5 py-2 text-left transition-all duration-200', collapsed ? 'justify-center px-2' : '', active ? 'bg-[var(--dv-nav-active)] text-[var(--dv-text)]' : 'text-[var(--dv-muted)] hover:bg-[var(--dv-nav-hover)] hover:text-[var(--dv-text)]')}>
    ...
    <span className={cn('grid h-8 w-8 shrink-0 place-items-center rounded-[9px] transition-colors', active ? 'bg-[var(--dv-accent-soft)] text-[var(--dv-accent)]' : 'bg-[var(--dv-icon-bg)] text-current')}>{React.cloneElement(item.icon as React.ReactElement, { size: 17, strokeWidth: 1.8 })}</span>
...
export const SuperAppSidebar: React.FC<SuperAppSidebarProps> = ({ collapsed, setCollapsed, sidebarVisible, isMobile, sidebarOpen, user, logout, toggleSidebar, isGuest = false, pendingApprovals = 0, isAdmin = false }) => {
  const navigate = useNavigate(); const location = useLocation(); const { t } = useTranslation(); const [moreOpen, setMoreOpen] = React.useState(false); const { open: openCommandPalette } = useCommandPalette();
  const text = React.useCallback((key: string, fallback: string) => { const value = t(key); return value && value !== key ? value : fallback; }, [t]);
  const go = (path: string) => { navigate(path); if (isMobile) toggleSidebar(); };
  const width = collapsed ? 76 : 260; const visible = sidebarVisible || (isMobile && sidebarOpen); if (!visible) return null;
  return <>
    ...
    <motion.aside initial={false} animate={{ width: isMobile ? 280 : width, x: isMobile && !sidebarOpen ? -300 : 0 }} transition={{ type: 'spring', stiffness: 360, damping: 34 }} className="fixed inset-y-0 left-0 z-50 flex flex-col overflow-hidden border-r border-[var(--dv-border)] bg-[var(--dv-sidebar)]">
      ...
      <div className="px-3 pt-3"><button type="button" onClick={openCommandPalette} className={cn('flex w-full items-center gap-2.5 rounded-xl border border-[var(--dv-border)] bg-[var(--dv-surface)] px-2.5 py-2 text-left transition hover:border-[var(--dv-border-strong)]', collapsed && !isMobile && 'justify-center')} aria-label={text('common.search', 'Поиск')}><span className="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-[var(--dv-accent-soft)] text-[var(--dv-accent)]"><Command size={14} /></span>{(!collapsed || isMobile) && <><span className="flex-1 text-xs font-medium text-[var(--dv-text)]">{text('common.search', 'Поиск')}</span><kbd className="rounded-md border border-[var(--dv-border)] px-1.5 py-0.5 text-[10px] text-[var(--dv-muted)]">⌘K</kbd></>}</button></div>
      <nav aria-label={text('nav.main_nav', 'Главная навигация')} className="min-h-0 flex-1 overflow-y-auto px-3 py-4 [scrollbar-width:none]">
        {groups.map((group) => <section key={group.id} className="mb-5">{(!collapsed || isMobile) && <div className="mb-1.5 px-2 text-[10px] font-semibold tracking-[0.14em] text-[var(--dv-muted-2)]">{text(group.labelKey, group.fallback)}</div>}{collapsed && !isMobile && <div className="mx-auto mb-1.5 h-px w-7 bg-[var(--dv-border)]" />}<div className="space-y-0.5">{group.items.map((item) => <NavItem key={item.id} item={item} label={text(item.labelKey, item.fallback)} collapsed={collapsed && !isMobile} active={isActive(location.pathname, item.path)} onNavigate={go} />)}</div></section>)}
        <section>{(!collapsed || isMobile) && <div className="mb-1.5 px-2 text-[10px] font-semibold tracking-[0.14em] text-[var(--dv-muted-2)]">{text('nav.menu', 'Меню')}</div>}<button type="button" onClick={() => setMoreOpen(!moreOpen)} aria-expanded={moreOpen} className={cn('flex w-full items-center gap-3 rounded-[11px] px-2.5 py-2 text-left text-[13px] font-medium text-[var(--dv-muted)] hover:bg-[var(--dv-nav-hover)] hover:text-[var(--dv-text)]', collapsed && !isMobile && 'justify-center')}><span className="grid h-8 w-8 place-items-center rounded-[9px] bg-[var(--dv-icon-bg)]"><Menu size={17} /></span>{(!collapsed || isMobile) && <><span className="flex-1">{text('nav.menu', 'Меню')}</span>{moreOpen ? <ChevronDown size={15} /> : <ChevronRight size={15} />}</>}</button><AnimatePresence initial={false}>{moreOpen && (!collapsed || isMobile) && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="mt-0.5 overflow-hidden pl-2">{moreItems.map((item) => <NavItem key={item.id} item={item} label={text(item.labelKey, item.fallback)} collapsed={false} active={isActive(location.pathname, item.path)} onNavigate={go} />)}</motion.div>}</AnimatePresence></section>
