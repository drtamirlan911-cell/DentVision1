...
import { User, Phone, CalendarDays, CreditCard, FilePlus, Search, Stethoscope, Heart, Shield, AlertCircle, CheckCircle, GraduationCap, Store, Sparkles } from 'lucide-react'
...
interface PatientContextData { id: string; name: string; phone?: string; nextVisit?: string; treatmentStage?: string; debt?: number; avatar?: string; notes?: string; allergies?: string[]; insurance?: string }
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
        <GlassCard padding="md" className="border-dv-gold/15">
          <div className="flex items-start gap-3">
            <div className="h-12 w-12 sm:h-14 sm:w-14 rounded-xl bg-dv-gold/10 border border-dv-gold/15 flex items-center justify-center shrink-0 overflow-hidden">
              {patient.avatar ? <img src={patient.avatar} alt="" className="h-full w-full object-cover" /> : <User size={22} className="text-dv-gold" />}
            </div>
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-2 mb-1"><h3 className="text-sm font-semibold text-txt-primary truncate">{patient.name}</h3><Badge variant="outline" className={cn('text-[10px]', currentStage.color)}>{currentStage.label}</Badge></div>
              <div className="flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-txt-muted">
                {patient.phone && <span className="flex items-center gap-1"><Phone size={11} />{patient.phone}</span>}
                {patient.nextVisit && <span className="flex items-center gap-1"><CalendarDays size={11} />След.: {formatDate(patient.nextVisit)}</span>}
                {!!patient.debt && patient.debt > 0 && <span className="flex items-center gap-1 text-red-400"><CreditCard size={11} />Долг: {patient.debt.toLocaleString('ru-RU')} ₸</span>}
              </div>
            </div>
          </div>
        </GlassCard>
      </motion.div>
      <GlassCard padding="md"><h4 className="text-xs font-semibold text-txt-primary mb-3">Этапы лечения</h4><div className="flex justify-between gap-1">{TREATMENT_STAGES.map((stage, i) => { const Icon = stage.icon; const done = i < stageIndex; const current = i === stageIndex; return <div key={stage.id} className="flex flex-col items-center gap-1 flex-1"><div className={cn('h-8 w-8 rounded-lg border flex items-center justify-center', current && 'border-dv-gold bg-dv-gold/10', done && 'border-green-500/40 bg-green-500/10', !current && !done && 'border-white/10 bg-white/[0.03]')}>{done ? <CheckCircle size={14} className="text-green-400" /> : <Icon size={14} className={current ? stage.color : 'text-txt-muted'} />}</div><span className="text-[9px] text-txt-muted text-center leading-tight">{stage.label}</span></div> })}</div></GlassCard>
      <GlassCard padding="md"><h4 className="text-xs font-semibold text-txt-primary mb-2">Быстрые действия</h4><div className="grid grid-cols-1 sm:grid-cols-2 gap-2">{quickActions.map((a) => <Button key={a.label} size="sm" variant="secondary" className="w-full min-h-10" onClick={() => navigate(a.path)}>{a.label}</Button>)}</div></GlassCard>
      {(patient.allergies?.length || patient.notes || patient.insurance) && <GlassCard padding="md" className="space-y-2"><div>{!!patient.allergies?.length && <div><p className="text-[11px] text-txt-muted mb-1 flex items-center gap-1"><AlertCircle size={12} className="text-red-400" /> Аллергии</p><div className="flex flex-wrap gap-1">{patient.allergies.map((a) => <span key={a} className="px-2 py-0.5 rounded-full bg-red-500/15 text-red-400 text-[10px]">{a}</span>)}</div></div>}{patient.notes && <p className="text-xs text-txt-secondary">{patient.notes}</p>}{patient.insurance && <p className="text-[11px] text-txt-muted flex items-center gap-1"><Shield size={12} className="text-blue-400" /> {patient.insurance}</p>}</div></GlassCard>}
    </div>
  )
}
