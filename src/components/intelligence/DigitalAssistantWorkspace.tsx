          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground">
                <span className={cn('h-1.5 w-1.5 rounded-full', status === 'error' ? 'bg-destructive' : 'bg-emerald-500')} />
                {statusLabel}{clinic?.name ? ` · ${clinic.name}` : ''}
              </div>
            </div>
          </div>
          ...
          {displayMessages.length === 0 ? (
            <div className="mx-auto flex min-h-full w-full max-w-3xl flex-col justify-center px-5 py-10 md:px-8">
              <div className="mb-8">
                ...
                <h1 className="text-2xl font-semibold tracking-tight md:text-3xl">Чем помочь?</h1>
                <p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">Один диалог для клиники, пациентов, диагностики, материалов и рабочих задач.</p>
              </div>
              <div className="grid gap-2 sm:grid-cols-2">
                {STARTER_PROMPTS.map(prompt => (
                  <button key={prompt} onClick={() => send(prompt)} className="group rounded-xl border border-border bg-card px-4 py-3 text-left text-sm transition hover:border-primary/30 hover:bg-muted/40">
                    <span className="block pr-5 text-foreground">{prompt}</span>
                    <span className="mt-1 block text-xs text-muted-foreground opacity-0 transition group-hover:opacity-100">Спросить AI →</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="mx-auto w-full max-w-3xl px-4 py-6 md:px-8 md:py-8">
              <div className="space-y-5">
                {displayMessages.map(message => (
                  <ChatMessage key={message.id} message={message} onAction={(query: string) => {
              ...
              {suggestions.length > 0 && status === 'idle' && (
                <div className="mt-5 flex flex-wrap gap-2">{suggestions.slice(0, 4).map(s => <button key={s.id} onClick={() => send(s.label)} className="rounded-full border border-border px-3 py-1.5 text-xs text-muted-foreground hover:bg-muted">{s.label}</button>)}</div>
              )}
            </div>
          )}
        </section>

        <div className="shrink-0 border-t border-border/50 bg-background/95 px-4 pb-[max(12px,env(safe-area-inset-bottom))] pt-3 backdrop-blur md:px-6">
          <div className="mx-auto w-full max-w-3xl">
            <AIInputArea onSend={send} disabled={status === 'thinking' || status === 'executing'} status={status === 'confirmation' ? 'executing' : status} progress={progress} placeholder="Спросите ассистента…" />
            <div className="mt-2 text-center text-[10px] text-muted-foreground">AI может ошибаться. Проверяйте клинические решения и данные пациента.</div>
          </div>
        </div>
      </main>
