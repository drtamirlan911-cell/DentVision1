...
export function AIWorkspaceIndex({ onNavigate }: Props) {
  ...
  useEffect(() => {
    if (status === 'idle' || status === 'result') {
      setSuggestions(getSmartSuggestions({ user, guest: isGuest, pathname: location.pathname, focusType: context.focusType }).slice(0, 4))
    }
  ...
  const runAction = useCallback(async (raw: any, forceConfirmed = false) => {
    ...
    try {
      ...
      setErrorMessage(error?.message || t('ai.error_execution'))
      setAIStatus('error')
  ...
  const handleConfirm = useCallback(async (confirmed: boolean) => {
    ...
    await runAction({ ...action, confirmed: true }, true)
  ...
  const handleSend = useCallback(async (text: string) => {
    ...
    try {
      const opts = { userId: user?.id, clinicId, pathname: location.pathname, focusType: context.focusType, focusId: context.focusId }
      let response: any = null
      response = await aiChatStream(value, historyRef.current.slice(-20), (partial, done) => {
        if (partial && !done) {
          useAIStore.setState(state => {
            const existing = state.messages.find(m => m.id === 'ai-stream')
            const msg = { id: 'ai-stream', role: 'assistant' as const, content: partial, timestamp: new Date() }
            return { messages: existing ? state.messages.map(m => m.id === 'ai-stream' ? msg : m) : [...state.messages, msg] }
          })
        }
      }, opts as any)
      ...
      if (response?.conversationContext?.entities) {
        const entities = response.conversationContext.entities
        if (entities.patientId) setContextFocus('patient', String(entities.patientId), entities)
      }
      if (Array.isArray(response?.suggestions)) setSuggestions(response.suggestions.slice(0, 4))
      const actions = Array.isArray(response?.actions) ? response.actions : []
      setAIStatus('result')
      if (actions.length) await runAction(actions[0])
      window.setTimeout(() => setAIStatus('idle'), 1200)
    } catch (error: any) {
      setErrorMessage(error?.message || t('ai.generic_error'))
      setAIStatus('error')
      window.setTimeout(() => setAIStatus('idle'), 2500)
    } finally { setProgress(0) }
  }, [status, addMessage, setAIStatus, setProgress, user?.id, clinicId, location.pathname, context, voice, setSuggestions, setContextFocus, runAction, setErrorMessage, t])
