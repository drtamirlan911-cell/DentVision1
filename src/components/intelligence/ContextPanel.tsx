...
export function ContextPanel({ onClose }: ContextPanelProps) {
  ...
  async function load() {
    setLoading(true);
    try {
      const [twinData, alertData, contextData] = await Promise.allSettled([
        aiDigitalTwin(),
        aiProactive(),
        aiChat('context', []),
      ]);
      ...
      if (contextData.status === 'fulfilled' && contextData.value?.reply) {
        // Parse context from AI response
      }
      // Also fetch real clinic context
      fetchClinicContext();
    } catch {
      // ignore
    } finally {
  ...
  const fetchClinicContext = async () => {
    try {
      const { aiChat } = await import('@/utils/api');
      const res = await aiChat('Покажи контекст клиники: расписание, пациенты, финансы, лаборатория', []);
      // Parse response
    } catch {
      // ignore
    }
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="h-12 border-b border-bdr-subtle flex items-center justify-between px-3 shrink-0">
        <div className="flex gap-1">
          {[
            ...
            { id: 'alerts', label: 'Оповещения', icon: <Calendar size={12} /> },
          ].map((tab) => (
            <button
              key={tab.id}
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {activeTab === 'context' && (
          <ContextTab context={context} twin={twin} clinic={clinic} user={user} loading={loading} />
        )}
...
function ContextTab({ context, twin, clinic, user, loading }: { 
      ...
      >
        ...
        <div className="space-y-1.5">
          <div className="flex justify-between text-xs">
            ...
            <span className="text-txt-primary font-medium truncate pr-2">{clinic?.name || '—'}</span>
          </div>
          <div className="flex justify-between text-xs">
            <span className="text-txt-muted">Пользователь</span>
            <span className="text-txt-primary font-medium truncate pr-2">{user?.name || user?.login || '—'}</span>
          </div>
          <div className="flex justify-between text-xs">
