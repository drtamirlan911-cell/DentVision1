...
function ActionCard({ item, onNavigate }: { item: WorkspaceAction; onNavigate: (path: string) => void }) {
  ...
  return <button type="button" onClick={() => onNavigate(item.path)} className="group flex min-h-24 flex-col justify-between rounded-2xl border border-border bg-surface p-4 text-left transition hover:-translate-y-0.5 hover:border-primary/40 hover:bg-muted/40"><div className="flex items-start justify-between gap-3"><span className="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-primary/10 text-primary"><Icon size={18} /></span><ArrowUpRight size={16} className="text-muted-foreground transition group-hover:text-foreground" /></div><div><div className="mt-3 text-sm font-semibold text-foreground">{item.label}</div><div className="mt-1 line-clamp-2 text-xs text-muted-foreground">{item.description}</div></div></button>;
}
...
export default function EcosystemQuickActions({ limit = 6 }: { limit?: number }) {
  ...
  return <section><div className="mb-3 flex items-end justify-between gap-3"><div><h3 className="text-base font-semibold text-foreground">Быстрые действия</h3><p className="mt-1 text-sm text-muted-foreground">Следующие действия для вашего рабочего контекста.</p></div></div><div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">{workspace.actions.slice(0, limit).map(item => <ActionCard key={item.id} item={item} onNavigate={navigate} />)}</div></section>;
}
