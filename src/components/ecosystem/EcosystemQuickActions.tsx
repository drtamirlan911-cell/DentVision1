...
import { ArrowUpRight, Bot, BriefcaseBusiness, Building2, FlaskConical, GraduationCap, Package, Users } from 'lucide-react';
...
function ActionCard({ item, onNavigate }: { item: WorkspaceAction; onNavigate: (path: string) => void }) {
  ...
  return (
    <button
      ...
      className="group flex min-h-24 flex-col justify-between rounded-2xl border border-border bg-surface p-4 text-left transition hover:-translate-y-0.5 hover:border-primary/40 hover:bg-muted/40"
    >
    ...
    <section>
      <div className="mb-3 flex items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Следующие действия для вашего рабочего контекста.</p>
        </div>
      </div>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {workspace.actions.slice(0, limit).map((item) => (
