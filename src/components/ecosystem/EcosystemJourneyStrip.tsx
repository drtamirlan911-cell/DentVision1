...
export const EcosystemJourneyStrip: React.FC<Props> = ({ journeyIds, className }) => {
  ...
  return <section className={cn('space-y-2', className)}>
    ...
    <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
      {journeys.map(journey => <button key={journey.id} type="button" onClick={() => navigate(withEcosystemContext(`/?journey=${encodeURIComponent(journey.id)}`, context))} className="group rounded-2xl border border-bdr-subtle bg-surface-1 p-3 text-left transition hover:border-dv-gold/30 hover:bg-surface-2">
