        ...
        <CardContent className="p-4 md:p-5 space-y-4">
          <div className="flex items-start justify-between gap-3 flex-wrap">
            <div className="space-y-1 min-w-0">
              ...
              {amountLabel && <p className="text-xl font-bold text-dv-gold m-0 tabular-nums">{amountLabel}</p>}
            </div>
          </div>

          <ol className="m-0 pl-4 space-y-1 text-xs text-txt-muted list-decimal">
            <li>{t('payment.qr_step1')}</li><li>{t('payment.qr_step2')}</li><li>{t('payment.qr_step3')}</li>
          </ol>
          ...
          {qrUrl ? (
            <div className="grid grid-cols-1 sm:grid-cols-[auto_1fr] items-center gap-4 rounded-xl border border-bdr-subtle bg-surface-2 p-3 sm:p-4">
              ...
              <div className="flex-1 space-y-3 w-full min-w-0">
                <p className="text-xs text-txt-muted m-0">{t('payment.qr_scan_hint')}</p>
                ...
                <a href={qrUrl} target="_blank" rel="noreferrer" className="block text-[11px] text-dv-gold/90 underline break-all">{qrUrl}</a>
              </div>
            </div>
          ) : (
            <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2.5 text-xs text-amber-200/90">
              ...
              {payment?.id ? <span className="block mt-1 text-txt-muted">{t('payment.invoice_id')}: {String(payment.id)}</span> : null}
            </div>
          )}
