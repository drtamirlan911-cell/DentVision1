...
const intents = [
  { id: 'patient', title: 'Записаться к врачу', subtitle: 'Найти помощь и запись', icon: HeartPulse, path: '/login?role=patient' },
        ...
        <section className="flex flex-1 flex-col pt-[28px] md:justify-center md:pt-0">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.32 }}>
            <div className="flex h-8 w-full items-center rounded-2xl border border-bdr bg-dv-gold-muted px-3.5">
              <Sparkles size={13} className="mr-2 text-dv-gold" />
              <span className="text-[9px] uppercase leading-3 text-dv-gold">AI Operating System for Dentistry</span>
            </div>

            <h1 className="mt-6 max-w-[720px] text-4xl font-normal leading-[1.12] tracking-[-0.03em] md:text-6xl">
              Ваша стоматология.<br />В одной системе.
            </h1>
            <p className="mt-3 max-w-[620px] text-[13px] leading-[1.5] text-txt-secondary md:text-sm">
              AI, клиника, пациенты, диагностика, обучение и Shop — в одном рабочем пространстве.
            </p>

            <button type="button" onClick={() => navigate('/ai')} className="mt-8 flex h-14 w-full max-w-[620px] items-center justify-between rounded-2xl bg-dv-gold px-[18px] text-sm font-medium text-dv-gold-on transition-transform active:scale-[0.99]">
            ...
            <div className="mt-7 max-w-5xl">
              <div className="mb-3 flex items-end justify-between gap-3 px-1">
                <div>
                  <p className="text-[11px] font-semibold text-txt-primary">Что вы хотите сделать?</p>
                  <p className="mt-0.5 text-[10px] text-txt-muted">Начните с задачи — роль можно выбрать позже.</p>
                </div>
              </div>
            ...
            <div className="mt-5 max-w-[620px] border-t border-bdr-subtle pt-4">
              <p className="mb-2 px-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-txt-muted">Для работы в DentVision</p>
              <div className="grid grid-cols-2 gap-2.5">
                {workRoles.map(role => {
                  const Icon = role.icon
                  return (
                    <button key={role.id} type="button" onClick={() => navigate(role.path)} className="flex min-h-12 items-center gap-2.5 rounded-xl border border-bdr bg-surface-1 px-3 text-left transition-colors hover:border-dv-gold/35 hover:bg-surface-raised">
                      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-surface-2 text-dv-gold"><Icon size={16} strokeWidth={1.7} /></span>
                      <span className="min-w-0"><span className="block text-[11px] font-medium text-txt-primary">{role.title}</span><span className="block text-[9px] text-txt-muted">{role.subtitle}</span></span>
                    </button>
                  )
                })}
              </div>
