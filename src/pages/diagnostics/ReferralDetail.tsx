...
export default function ReferralDetail() {
  ...
  const askNextStep = async () => {
    ...
    const contextLines = [
      `Пациент: ${patient}.`,
      referral?.studyType ? `Исследование: ${referral.studyType}.` : '',
      referral?.category ? `Категория: ${referral.category}.` : '',
      referral?.complaints ? `Жалобы: ${referral.complaints}.` : '',
      referral?.preliminaryDx ? `Предварительный диагноз из направления: ${referral.preliminaryDx}.` : '',
      referral?.allergies ? `Аллергии/ограничения: ${referral.allergies}.` : '',
      `Результат исследования: ${reportText}.`,
