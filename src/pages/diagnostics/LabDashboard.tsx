...
export default function LabDashboard() {
  ...
  const { data: referralsData, isLoading, refetch } = useQuery({
    ...
    queryFn: () => api.getDiagnosticReferrals({ labId: selectedLabId, status: statusFilter, search, limit: '100' }),
    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-txt-muted mt-0.5">Управление входящими заказами на лабораторные исследования</p>
        </div>
        ...
        <>
          ...
          <Card padding="md">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-txt-primary">Заказы ({total})</h3>
            </div>
            {isLoading ? (
              ...
              <div className="flex items-center justify-center h-40 text-txt-muted text-sm flex-col gap-2">
                ...
                Нет заказов для этой лаборатории
              </div>
            ) : (
                    ...
                    {referrals.map((r: any) => {
                      ...
                      return (
                        <tr key={r.id} className="border-b border-bdr-subtle/50 hover:bg-surface-1/30 transition-colors">
                          ...
                          <td className="py-2.5 pr-3 text-txt-muted">{r.studyType || r.labTestType || '—'}</td>
                          ...
                          <td className="py-2.5 pr-3">
                            <div className="flex items-center gap-1">
                              ...
                              <Button size="xs" variant="ghost" aria-label="View" icon={<Eye size={14} />} onClick={() => navigate(`/diagnostics/referrals/${r.id}`)} />
                            </div>
                          </td>
                        </tr>
                      );
      ...
      {!selectedLabId && (
        <div className="flex items-center justify-center h-40 text-txt-muted text-sm flex-col gap-2">
          ...
          Выберите лабораторию для просмотра заказов
        </div>
      )}
    </motion.div>
  );
