...
export default function ResultList() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="dv-page space-y-6 max-w-full overflow-x-hidden">
      <PageHeader title="Результаты исследований" subtitle="Завершённые и просмотренные исследования" icon={<ClipboardList size={22} />} />
      <div className="dv-panel p-3 sm:p-4">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          <label className="relative flex-1 min-w-0" aria-label="Поиск по пациенту"><Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-txt-muted pointer-events-none" /><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Поиск по пациенту..." className="w-full min-h-11 bg-surface-raised border border-bdr-subtle rounded-lg pl-9 pr-3 py-2.5 text-sm text-txt-primary placeholder:text-txt-muted focus:outline-none focus:border-dv-gold/50 focus:ring-1 focus:ring-dv-gold/20 transition-colors" /></label>
