...
export default function MyClinics() {
  ...
  useEffect(() => {
    // Clinics are hydrated by AuthStore. Do not render or mutate this workspace
    // until authentication restoration has completed.
    if (!authLoading) setLoading(false);
  }, [authLoading]);

  // This route sits outside IntelligenceLayout, so it must enforce its own
  // authentication boundary. A guest token is never authorization for clinic data.
    ...
    <div className="min-h-screen bg-surface-0 p-6 relative max-w-full overflow-x-hidden">
      ...
      <div className="max-w-[760px] mx-auto relative z-10 space-y-8">
          ...
          <div>
            ...
            <div className="space-y-2.5">
              {clinics.map((m, i) => (
                ...
                >
                  <div className="w-11 h-11 rounded-xl flex items-center justify-center text-lg font-bold shrink-0" style={tintedAccent(m.clinic?.color, 13)}>
        ...
        <div className="p-4 bg-surface-1 border border-bdr-subtle rounded-xl flex flex-wrap items-start gap-3">
          ...
          <p className="text-xs text-txt-secondary leading-relaxed m-0">Не хотите создавать клинику? Вы уже можете пользоваться <span className="text-dv-gold">Магазином</span>, <span className="text-dv-gold">Академией</span> и <span className="text-dv-gold">AI-ассистентом</span> в личном режиме. CRM активируется только после выбора рабочего пространства.</p>
        </div>
      </div>
      ...
      <Modal open={activeTab === 'create'} onClose={() => setActiveTab('list')} title="Создание клиники" className="w-full max-w-full sm:max-w-md md:max-w-lg lg:max-w-xl">
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3"><Input label="Город" value={createForm.city} onChange={(e) => setCreateForm(f => ({ ...f, city: e.target.value }))} placeholder="Алматы" /><Input label="Страна" value={createForm.country} onChange={(e) => setCreateForm(f => ({ ...f, country: e.target.value }))} placeholder="Казахстан" /></div>
          ...
          <div className="flex flex-wrap gap-3 pt-2"><Button variant="ghost" onClick={() => setActiveTab('list')}>Отмена</Button><Button variant="primary" loading={creating} onClick={handleCreate} icon={<Building2 size={15} />}>Создать клинику</Button></div>
        </div>
      </Modal>
      ...
      <Modal open={activeTab === 'join'} onClose={() => setActiveTab('list')} title="Присоединиться к клинике" className="w-full max-w-full sm:max-w-md md:max-w-lg lg:max-w-xl">
        <div className="space-y-3">
          <div><label className="text-xs text-txt-secondary mb-1.5 block">Код приглашения</label><div className="flex gap-2"><Input value={joinCode} onChange={(e) => setJoinCode(e.target.value)} placeholder="ABCD-1234" icon={<KeyRound size={15} />} /></div></div>
          <div className="flex items-center gap-2 text-xs text-txt-muted"><QrCode size={14} /> <Link2 size={14} /> Также можно присоединиться по ссылке-приглашению</div>
          <div className="flex flex-wrap gap-3 pt-2"><Button variant="ghost" onClick={() => setActiveTab('list')}>Отмена</Button><Button variant="primary" loading={joining} onClick={handleJoin} icon={<LogIn size={15} />}>Присоединиться</Button></div>
        </div>
      </Modal>
    </div>
  );
...
function ActionCard({ icon, title, desc, tone, onClick, loading }: { icon: React.ReactNode; title: string; desc: string; tone: keyof typeof ACTION_TONES; onClick: () => void; loading?: boolean }) {
  return (
    <motion.button whileHover={{ y: -3 }} disabled={loading} onClick={onClick} className="p-5 min-h-11 bg-surface-1 border border-bdr-subtle rounded-[14px] text-left cursor-pointer hover:border-dv-gold/40 transition-all flex flex-col gap-2 disabled:opacity-60 disabled:cursor-not-allowed">
      <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${ACTION_TONES[tone]}`}>{icon}</div>
      <div><p className="text-sm font-bold text-txt-primary m-0">{title}</p><p className="text-xs text-txt-muted m-0 mt-0.5">{desc}</p></div>
