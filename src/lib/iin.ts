 ...
 *   12    check digit
 *
 * Pure module, no imports, no I/O — mirrored verbatim from `dentvision-backend/src/lib/iin.ts` for
 * the browser. The two are asserted to agree over a generated corpus in
 * the backend `iin.test.ts`. Duplicating a checksum is not free, but the browser
 * must reject a typo without a round trip and the two projects do not share
 * a build.
 */
