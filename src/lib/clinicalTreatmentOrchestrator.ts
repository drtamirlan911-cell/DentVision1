...
/**
 * Deterministic workflow planner. It never mutates clinical data and never
 * invents a diagnosis. Mutating operations are represented as explicit,
 * confirmation-gated intents for the existing AI tool/executor layer.
 */
export function buildClinicalTreatmentWorkflow(
  context: ClinicalCaseContext,
): ClinicalTreatmentStep[] {
  ...
  return [
    ...
    {
      id: 'treatment-plan',
      title: hasPlan ? 'Проверить план лечения' : 'Подготовить план лечения',
      ...
      actionType: 'OpenTreatmentPlans',
      status: hasPlan ? 'ready' : 'available',
      requiresConfirmation: !hasPlan,
    },
    {
      id: 'schedule',
      title: hasVisit ? 'Проверить визит' : 'Запланировать визит',
      description: hasVisit ? 'Открыть связанный визит.' : 'Создание записи изменяет расписание и требует подтверждения.',
      actionType: 'OpenSchedule',
      status: hasVisit ? 'ready' : 'available',
      requiresConfirmation: !hasVisit,
    },
    {
      id: 'laboratory',
      title: 'Лаборатория',
    ...
    {
      id: 'payment',
      title: hasDebt ? 'Закрыть финансовый вопрос' : 'Проверить оплату',
      ...
      actionType: 'OpenCashier',
      status: hasDebt ? 'available' : 'ready',
      requiresConfirmation: hasDebt,
    },
    {
      id: 'follow-up',
      title: 'Контроль и follow-up',
...
export function workflowStepToAction(
  ...
  return {
    id: `clinical-workflow-${step.id}`,
    type: step.actionType,
    label: step.title,
    confidence: 1,
    requiresConfirmation: step.requiresConfirmation,
    params: {
      path: params.toString() ? `${path}?${params.toString()}` : path,
      ...context,
    },
  }
}
