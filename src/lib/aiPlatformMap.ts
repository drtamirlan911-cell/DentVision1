...
export function getSmartSuggestions(opts: {
  ...
  const byStage: Record<string, string[]> = {
    schedule: clinical
      ...
      : ['Кто сегодня?', 'Записать пациента', 'Что важно сегодня?'],
    finance: clinical
  ...
  return clinical
    ? [...new Set([...roleChips, ...(byStage[stage] || [])])].slice(0, 4)
    : [...new Set([...(byStage[stage] || []), ...roleChips])].slice(0, 4)
}
