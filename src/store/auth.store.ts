...
async function applySignIn(set: (partial: Partial<AuthState>) => void, result: any): Promise<void> {
  ...
  set({ user, token: accessToken, refreshToken, clinic: buildClinicFromMembership(activeMembership), clinics: memberships, workspaceContexts: (result.workspaceContexts || get().workspaceContexts || []) as WorkspaceContext[], activeWorkspace: (result.workspaceContexts || []).find((w: WorkspaceContext) => w.scopeId === (user as any)?.organizationOriginalId || w.scopeId === (user as any)?.clinicId) || get().activeWorkspace, activeMembership, activeClinic: buildClinicFromMembership(activeMembership), permissions, pages, effectiveRole, capabilities, loading: false, error: null })
