...
interface AuthState {
  user: User | null; token: string | null; refreshToken: string | null; clinic: Clinic | null; clinics: Membership[]; activeMembership: Membership | null; activeClinic: Clinic | null; permissions: string[]; pages: string[]; effectiveRole: string | null
  ...
  login: (loginStr: string, password: string) => Promise<boolean>; loginWithGoogle: (idToken: string) => Promise<boolean>; logout: () => void; register: (formData: RegisterFormData) => Promise<boolean>; forgotPassword: (loginStr: string) => Promise<unknown>; refresh: () => Promise<void>; restoreSession: () => Promise<void>; setActiveClinic: (clinicId: string) => Promise<void>; switchClinic: (clinicId: string | null) => Promise<void>; addStaffMember: (staffData: StaffData) => Promise<User | false>; getClinicStaff: (clinicId: string) => User[]
...
async function applySignIn(set: (partial: Partial<AuthState>) => void, result: any): Promise<void> {
  ...
  set({ user, token: accessToken, refreshToken, clinic: buildClinicFromMembership(activeMembership), clinics: memberships, activeMembership, activeClinic: buildClinicFromMembership(activeMembership), permissions, pages, effectiveRole, capabilities, loading: false, error: null })
  useGuestStore.getState().clearGuest()
}

export const useAuthStore = create<AuthState>((set, get) => ({
  user: null, token: null, refreshToken: null, clinic: null, clinics: [], activeMembership: null, activeClinic: null, permissions: [], pages: [], effectiveRole: null,
  ...
  switchClinic: async (clinicId) => { try { const result = await api.switchClinic(clinicId); if (result?.accessToken) api.setTokens(result.accessToken, result.refreshToken ?? null); let activeMembership = mapActiveMembership(result?.activeMembership); if (!activeMembership && clinicId) activeMembership = get().clinics.find((m) => m.clinicId === clinicId) || null; if (!activeMembership) { const me = await hydrateAuthFromMe(); activeMembership = pickActiveMembership(clinicId ? me.memberships.find((m) => m.clinicId === clinicId) || me.activeMembership : me.activeMembership, me.memberships); set({ user: me.user, clinics: me.memberships, token: result?.accessToken || get().token, refreshToken: result?.refreshToken ?? get().refreshToken, activeMembership, clinic: buildClinicFromMembership(activeMembership), activeClinic: buildClinicFromMembership(activeMembership), permissions: me.permissions, pages: me.pages, effectiveRole: me.effectiveRole || null, capabilities: me.capabilities }); return } set({ token: result?.accessToken || get().token, refreshToken: result?.refreshToken ?? get().refreshToken, activeMembership, clinic: buildClinicFromMembership(activeMembership), activeClinic: buildClinicFromMembership(activeMembership), permissions: Array.isArray(result?.permissions) ? result.permissions : get().permissions, pages: Array.isArray(result?.pages) ? result.pages : get().pages, effectiveRole: result?.effectiveRole || get().effectiveRole, capabilities: result.capabilities || get().capabilities }) } catch (err) { set({ error: (err as Error).message || 'Failed to switch clinic' }); throw err } },
...
export function useAuth() {
  ...
  return { user, clinic, clinics, activeMembership, activeClinic, mode, loading, error, login, loginWithGoogle, logout, register, forgotPassword, addStaffMember, getClinicStaff, switchClinic, isAuthenticated: !!user, role, roleInfo, pages, capabilities, effectiveRole, permissions, can, allClinics, allUsers }
}
