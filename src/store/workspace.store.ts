...
export const useWorkspaceStore = create<WorkspaceState>((set) => ({
  ...
  setOnboardingComplete: (completed) => {
    try { if (completed) sessionStorage.setItem('dv_welcomed', '1') } catch { /* ignore */ }
    set((state) => ({ onboarding: { ...state.onboarding, completed } }))
  },
  setOnboardingScreen: (screen) => set((state) => ({ onboarding: { ...state.onboarding, currentScreen: screen } })),
  setOnboardingSkipped: (skipped) => set((state) => ({ onboarding: { ...state.onboarding, skipped } })),
}))
