...
- [x] Все 7 обязательных разделов доступны в production navigation
- [x] Создание записи, пациента, плана, оплаты — из UI и из AI (navigate + tool loop)
- [~] Зубная карта связана с планом и визитом (teeth persist + plan stages; deep links)
- [x] Склад умеет порождать заказ в Marketplace (deep-link из критичных остатков)
- [x] Документ генерируется из визита и уходит на подпись (JWT send/sign)
- [~] Автонапоминания и долги работают без ручного экспорта в Excel (отчёты + WhatsApp deep-link; SMS cron — next)
- [x] Нет сценария «данные есть в одном разделе и невидимы в связанном» для waiting list / expenses / promotions / appointments meta
